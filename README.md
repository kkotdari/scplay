...
**장면 스크랩은 버튼째 낸다**(요청: "버튼은 css까지 먹여서 네가 만들어서 기본값 제공해 줘야지 —
갖다 쓰는 쪽은 사용 여부 판단하고 기능만 붙이는 거고"). `onScrap`을 주면 진행바 아래 꼬리 줄에
공유·사용법과 **같은 꼴의 알약**으로 서고, 단축키 `Z`와 완료 표시("담았어요")까지 모듈이 진다.
앱이 지는 것은 담는 일뿐이다 — 참을 돌려주면(또는 참으로 풀리는 약속이면) 완료 표시가 뜬다.
글씨를 바꾸려면 `scrapLabel`. 안 주면 버튼이 아예 안 선다.
