...
**스크랩·공유는 버튼째 낸다**(요청: "버튼은 css까지 먹여서 네가 만들어서 기본값 제공해 줘야지 —
쓰는 쪽에서 쓸지 말지 선택하는 거고 함수도 알아서 연결해야 해"). 함수를 주면 진행바 아래 꼬리 줄에
**같은 꼴의 알약**으로 서고, 안 주면 안 선다 — 그것이 곧 사용 여부 판단이다.

| 프롭 | 버튼 | 단축키 | 앱이 지는 일 |
|---|---|---|---|
| `onScrap` | 장면 스크랩 | `Z` | 담기(제목 창·저장) |
| `onShare` | 장면 공유 | `X` | 공유(카카오·공유 시트·링크 복사) |

자리·꼴·아이콘·단축키·완료 표시는 모듈 몫이다. 단축키는 **홀로 있는 판**(`soleView`이거나
전체화면)에서만 듣는다 — 목록처럼 카드마다 재생기가 서는 화면에서 Z 한 번에 여럿이 답하지 않게. 완료 표시 규약은 하나다 — 함수가 **글**을 돌려주면
그 글이, **참**을 돌려주면 기본 글("담았어요"·"링크 복사됨")이 1.8초 뜬다. 약속이면 풀릴 때까지
기다린다. 글씨는 `scrapLabel`·`shareLabel`.
`shareNode` 슬롯도 그대로 남는다 — 제 꼴을 가진 버튼(카카오 알약 따위)을 꽂던 옛 길이다.
