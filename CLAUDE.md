...
- **WebGL 붓 `#gl=1`**(`gl9.ts`): 유닛·건물 몸을 **판 없이** 메시로 GPU 가 그린다. 메시는 열쇠별 한 벌(유닛 `u:종류:자세:lod`,
  건물 `b:종류:건설단계:포탑각:불빛:회전:lod` — rasterBld9 와 같은 깃발), 상한 600 벌(오래된 것부터 버림). 정점 셰이더가 `project` 와
  같은 카메라(평면 sin40/cos40 · 입체 pitchFlatNow·0.7/0.9/앞숙임 0.34/시각 밀림 tan(vq), `camOf9`)를 걸고, 자리·배수는 판 블릿과
  같은 자다(유닛 앵커 (sx, sy−px·0.24−lift)·px/16·MODEL_NORM·원점 줄 12/12.6 · 건물 x 상자 가운데, 잉크 바닥을 바닥선−띄움에).
  판이 주던 잉크 상자(그림자·링·체력바·BLD_INK_BOX 의 폭·가운데·바닥)는 `footOf`(메시 꼭짓점을 카메라·요잉으로 돌려 잰 상자)가
  같은 자로 준다. 앞뒤는 깊이 버퍼(개체 차례 칸 + 카메라 가까움), 조명은 법선 방향광 양면, 임자색은 uTeam, 몸 그림자는 검은 재그리기.
  GL 뒷캔버스(`.scr-motion-gl9`, display:none)는 붓이 몸을 다 큐에 넣은 뒤 **효과 전에** 유닛 캔버스에 합성한다.
  GL 이 켜지면 유닛·건물의 2D 면 굽기(resolveShapeFaces)·판 굽기(unitSprite/buildingSprite)는 안 돈다(데우기도 메시로).
  아직 캔버스가 맡는 것: 크립 판·얼룩·맨 네모·그림자 타원·링·체력바·효과. 2D 와 다른 점: 면 광택(#fff/#000 덧칠·silhouetteLight)이
  없어 살짝 평평하고, 몸 그림자 흐림이 없다. `#diag=draw` 의 'GL' 줄(개체·삼각·메시·판으로 떨어진 종류), `perf-check --hash gl=1 [--warm 0]`.
