...
- **WebGL 붓(어느 기기에서나 기본 켬 · `#gl=0` 이면 면을 곧장 그리는 느린 폴백)**(`gl9.ts`): 유닛·건물 몸을 **판 없이** 메시로 GPU 가 그린다.
  ...
  · **광택(스페큘러)**(2026-09, 요청: "셰이더에 스페큘러 항 넣어서 광택 되살려줘 … 금속뿐 아니라 저그 살점 윤택도 있고") —
    ...
    · **자는 종족이 든다**(`glossOf9`) — 낯마다 재질을 적으면 빌더 154종을 다 만져야 하고 색으로 짐작하면 틀린다.
      도록 표(`SHAPE_GALLERY`)가 154종을 이미 종족으로 갈라 두었다: 테란 [28, 0.70, 0.30, 0.04, 0.85] · 프로토스
      [32, 0.58, 0.70, 0.05, 0.35] · 저그 [14, 0.26, 0.15, 0.05, 0] · 그 밖 [17, 0.14, 0.30, 0.02, 0.15]
    ...
    · ★ 봉우리는 한 번 **1.45배 올렸다**(2026-09, 지적: "봉우리가 좀 강해야 할 거 같은데") — 허옇게 뜨는 것은
      **윤기**(넓은 쪽)지 봉우리가 아니다. 봉우리는 좁아 닿는 낯이 적으므로 세게 줘도 반사광 한 점으로 남는다
      (gl-check 평균 0.179 → 0.181 · 밝기비 1.098 → 1.112 · 저그 알·고치는 안 떴다).
    · gl-check 154종: 평균 나쁨 0.174 → **0.183** · 밝기비 1.044 → **1.117** · 0.5 넘는 종류 0. 2D 에 없는 빛이라
