...
- **WebGL 붓(어느 기기에서나 기본 켬 · `#gl=0` 이면 면을 곧장 그리는 느린 폴백)**(`gl9.ts`): 유닛·건물 몸을 **판 없이** 메시로 GPU 가 그린다.
  ...
  · **닫힌 입체의 뒷면은 버린다**(2026-09, mesh9.solidSigns9 · 정점 aBb 128): 뭉치(pid)의 폴리를 모아 ① 모든 모서리를 두 낯이
    ...
    부피로 안·바깥을 정한다. 반투명 뭉치는 속이 보여야 하므로 안 건다. **그림은 한 톨도 안 바뀐다**(gl-check 154종 전부
    같은 값) — 닫힌 덩이의 뒷면은 어차피 앞면이 깊이로 덮기 때문이다. 버는 것은 **화소 채우기**(닫힌 덩이 넓이의 절반)라
    폰의 GPU 몫이 그만큼 준다.
  ...
  · **전수조사 `node scripts/gl-check.mjs [--kinds a,b] [--rots 45,225] [--worst 40] --out <scratch>/glcheck.png [--json x.json]`** —
    ...
    GL 붓을 고치면 이걸로 154종을 다시 돈다(2분). 기준(2026-09, 방향광·바닥 그림자·덧칠 세기 뒤): 평균 나쁨 0.183 · 밝기비 1.06 ·
    나쁨 0.5 넘는 종류 0. ★ 이 자는 이제 **"2D 와 같은가"가 아니라 "빠진 부품이 없나"**를 보는 자다 — 방향광·테두리 빛은 2D 에 없는
    몫이라 색차가 조금 오르는 것이 정상이다. 남은 큰 어긋남(tankbarrel·geyser·assim·diamond·dmound: IoU 0.67~0.8)은 GL 화소가 2D 보다
    **적은** 자리다 — 되찾기가 안 되는 면(가스 기둥 같은 화면 원 겹침)이 빠진 것이라 따로 고칠 거리다.
