// 모델 변천사 — 세 시점(J 7월 2D · A 8/29 2D · C 지금 GL)의 칸을 한 줄에 셋 붙인다. 시트는 도록 일곱 장과 같은 묶음.
// ★★ 7월 열도 이제 **굽는다**(2026-09, 요청: "stargayte 레포지토리 … 7월에 쓰던 레포지토리야 · 이걸 이용해서 변천사 목록을
//    새롭게 뽑을 수 있을거야") — 옛 길은 사용자가 준 도록 그림에서 칸을 떼어 누끼를 딴 `scripts/history/july/*.png` 였다.
//    그 재료를 낸 코드가 딴 저장소(kkotdari/stargayte)에 살아 있어, 그 판을 그 시대 붓으로 다시 구워 옆 두 시대와 같은 자
//    (칸 400 · --fit 0.7 · 흰 바탕 · 임자색 #7ed491 · 접지 그림자)로 세운다. 곧 세 열이 **다 같은 규약**으로 굽힌 그림이다.
...
/* ★ 칸은 400px(2026-09, 지적: "변천사 화질이 너무 안좋아") — 세 시대를 다 400 으로 굽는다
   (7월·8/29 는 `hist-shot.mjs --cell 400 --fit 0.7` · 지금은 `model-gl --cell 400 --fit 0.7`).
   `--fit` 은 칸마다 잉크 상자를 칸의 그 몫에 맞추므로 아비터처럼 원 좌표가 작은 종류도 칸을 채운다. */
const NOW = "9/22";   // 지금 시점의 꼬리표(굽는 날)
const CELL = 400; const LAB = 170; const HEAD = 56; const GAP = 6;
const kindsU = readFileSync(`${S}/kinds_u.txt`, "utf8").trim().split(",");
const kindsB = readFileSync(`${S}/kinds_b.txt`, "utf8").trim().split(",");
/* ★★ **시대마다 제 카메라·제 붓이다**(2026-09, 요청: "8월은 +45도 요잉 9월은 +40도 요잉(각 시대 표준값) · 시대별 카메라
   시점 파라미터 갖기 · 임자색은 7월에 적용한 색(연녹색) 모든 시대에 · 배경 흰색으로 — 그림자 표현 변천사도 보여주고 싶어서")
   ── 판 여섯: histJ_u/b(7월 · stargayte 8ddd494 · 2D) · histA_u/b(8/29 · scplay 1e20b08 · 2D) · histC_u/b(지금 · GL).
   · 요잉: 7월·8/29 +45 · 지금 +40(BUILDING_BASE_YAW 와 같은 자).
     ⚠ 7월 시대의 **지도** 기본 요잉은 사실 0 이었다(그 판의 resolveShapeFaces 는 건물에 rotDeg 를 안 준다 · 도록의
       VIEW.yawDeg 도 0). 그런데 그 시대 도록은 칸마다 손으로 돌려 보는 자라 사용자가 준 7월 시트도 23°·45° 로 돌아가
       있었다 — 곧 '7월의 표준 한 각'이 없다. 꼴을 견주는 것이 이 도록의 뜻이라 옆 시대와 같은 **+45** 로 못 박았다
       (되물리려면 ERAS.J.yaw 한 값이다).
   · 임자색은 7월 도록의 연녹색 #7ed491 · 배경은 셋 다 흰색 · 그림자는 7월·8/29 가 접지 타원(hist-shot `--shadow`) ·
     지금은 gl9 의 사영 그림자(model-gl `--shadow`). */
export const ERAS = {
  J: { tag: "7월 · 2D", yaw: 45, u: "JU", b: "JB" },
...
/* ★ 7월·8/29 는 둘 다 **model-shot 사본**(hist-shot.mjs)이 낸 시트라 행 차례가 곧 kinds 목록의 차례다 —
   그 시대에 없던 종류는 칸이 비지만 **행은 선다**(KINDS 를 그대로 도는 자라 차례가 안 밀린다). */
const idxJ = new Map(); const idxA = new Map(); const idxC = new Map();
kindsU.forEach((k, i) => { idxJ.set(k, { img: "JU", i }); idxA.set(k, { img: "AU", i }); });
kindsB.forEach((k, i) => { idxJ.set(k, { img: "JB", i }); idxA.set(k, { img: "AB", i }); });
...
const imgs = {
  JU: b64("histJ_u"), JB: b64("histJ_b"),
  AU: b64("histA_u"), AB: b64("histA_b"),
  CU: b64("histC_u"), CB: b64("histC_b"),
};
...
const loaded = await pg.evaluate(async (imgs) => {
  window.__im = {};
  for (const k of Object.keys(imgs)) { const i = new Image(); i.src = imgs[k]; await i.decode(); window.__im[k] = i; }
...
/* ★ 판형(요청: "한 줄에 3모델" · 보기 그림) — 종족마다 한 장: '유닛'·'건물' 절 아래 모델 카드가 한 줄에 셋. 카드 안은 시점이
   왼쪽에서 오른쪽으로(7월 2D · 8/29 2D · 지금 GL) 선다. 칸마다 왼 위에 작은 꼬리표. */
...
for (const [file, race, nos] of RACES) {
  const secs = nos.map((no) => sheets.find((x) => x.no === no)).map((sh) => ({
    ...
    cards: sh.items.filter((it) => idxC.has(it.kind)).map((it) => ({ ...it, j: idxJ.get(it.kind), a: idxA.get(it.kind), c: idxC.get(it.kind) })),
  }));
  const data = await pg.evaluate(async ({ secs, CELL, COLS, TITLE, SEC, race, NOW, ERAS, BG9, OWN9, SPLIT }) => {
    /* ★ 카드 상자·칸 테두리는 걷었다(2026-09, 요청: "한 모델 안에서 세로 구분선 제거 · 제목은 각 모델 위에 줄 위에") —
       ...
       ① 붓의 strokeRect ② hist-shot 칸의 왼 가장자리
    ...
    c.fillStyle = "#6b7480"; c.font = "11px sans-serif"; c.fillText(`왼쪽부터 ${ERAS.J.tag}(+${ERAS.J.yaw}°) · ${ERAS.A.tag}(+${ERAS.A.yaw}°) · ${ERAS.C.tag}(+${ERAS.C.yaw}° · 지금) — 시대마다 그 시대의 붓 · 임자색 ${OWN9} · 흰 바탕(그림자가 보이게)`, 10, 28);
    ...
    for (const sc of secs) {
      ...
      sc.cards.forEach((r, i) => {
        ...
        const cells = [[`${ERAS.J.tag} +${ERAS.J.yaw}°`, "J", r.j], [`${ERAS.A.tag} +${ERAS.A.yaw}°`, "A", r.a], [`${ERAS.C.tag} +${ERAS.C.yaw}°`, "C", r.c]];
        cells.forEach(([tag, k, idx], n) => {
          ...
          if (idx != null) {
            if (k === "C") {
              ...
              /* 7월·8/29 는 같은 도구(hist-shot = model-shot 사본)가 낸 시트다 — 칸 자리도 자도 같다.
                 위·아래 두 줄이 격자선이라 2px 안쪽을 잘라 쓰고, 칸에 박힌 종류 이름은 바탕으로 덮는다. */
              c.drawImage(window.__im[idx.img], 2, idx.i * CELL + 26 + 2, CELL - 4, CELL - 4, x + 2, yy + 2, CELL - 4, CELL - 4);
              c.fillStyle = BG9; c.fillRect(x, yy, 96, 22);
            }
  ...
  }, { secs, CELL, COLS, TITLE, SEC, race, NOW, ERAS, BG9, OWN9, SPLIT });
...
await br.close();
writeFileSync(`${OUT}/README.txt`, `모델 변천 비교 — 종족마다 한 장(유닛·건물). 카드마다 왼쪽부터 7월(옛 저장소 stargayte 8ddd494 · 2D 붓 · +45°) · 8/29(scplay 1e20b08 · 2D 붓 · +45°) · ${NOW}(GL 붓 · +40° · 지금). 임자색 ${OWN9} · 흰 바탕.\n`);
