...
import { collectMesh9, MESH_DBG9 } from ${JSON.stringify(join(ROOT, "src/utils/mesh9"))};
export { MESH_DBG9 };
export function run(only, pose) {
  const out = {}; MESH_DBG9.keep = true;
  for (const kind of only ?? Object.keys(SHAPE_BUILDERS)) {
    const b = SHAPE_BUILDERS[kind]; if (!b) continue;
    poseSet9(pose); headYawSet(0);
    const r0 = MESH_DBG9.rehost; const m0 = MESH_DBG9.moved.length;
    try { out[kind] = collectMesh9(b); out[kind].rehost = MESH_DBG9.rehost - r0; out[kind].moved = MESH_DBG9.moved.slice(m0); } catch (e) { out[kind] = { err: String(e).slice(0, 80), parts: [], faces: 0, covered: 0, skipped: 0, blank: 0 }; }
  }
  poseSet9(0);
  return out;
}`;
...
const reh = Object.entries(res).filter(([, m]) => m.rehost > 0).map(([k, m]) => `${k} ${m.rehost}`);
if (reh.length) console.log(`덧칠 얹힐 몸 바뀜: ${reh.join(" · ")}`);
...
if (DUMP) {
  for (const [kind, m] of Object.entries(res)) {
    ...
    for (const d of m.moved ?? []) console.log(`  ↪ 얹힐 몸 바뀜 ${d}`);
    m.parts.forEach((p, i) => {
