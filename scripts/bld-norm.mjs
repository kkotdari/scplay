...
function bundle() {
  ...
  /* esbuild를 **직접** 부른다(수리: 윈도에서 `npx`가 ENOENT) — PATH의 npx는 npx.cmd라
     execFileSync가 못 찾는다. model-shot.mjs·model-norm.mjs와 같은 수법으로, 실행
     파일이면 그대로 돌리고 아니면 노드로 돌린다(플랫폼별 매직 넘버 판별). */
  ...
  const args = [src, "--bundle", "--format=iife", "--jsx=automatic",
