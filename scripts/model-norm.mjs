...
/* 지도에 실제로 그려지는 kind vs 도록(자료실 > 모델)에만 있는 kind.
   ...
   'tank'·'tanksiege' 합본은 화면에 한 번도 안 나온다 — 크기 손잡이가 그 이름으로
   걸리면 죽은 값이다(지적 G).
   ('carrierbay'는 목록에서 뺐다 — 인터셉터가 제 모델을 갖게 되면서 그 합본 자체가
    사라졌다.) */
export const GALLERY_ONLY = new Set(["tank", "tanksiege"]);
...
export const NORM_PAIR = { tankgun: "tankbody", tanksiegegun: "tanksiegebody", tanksiegelegs: "tanksiegebody", tanksiegelegsF: "tanksiegebody", tankturretxf: "tankbody" };
...
function inBrowser({ KINDS, MODES, BUCKETS, VQ_PROBE, SCALES, FOOT_Y, NORM_ANCHOR, TRIM_FLOOR,
  ...
  const shot = (g, kind, bucket, mode, vq, s) => {
    ...
    for (const f of faces) {
      ...
      if (f[6] & 1) continue;                    // 자 재기 제외 표식(boxSkip · 비트 1)
...
const launchOpt = { executablePath: exe, args: ["--no-proxy-server"] };
