...
const ENTRY = `
window.__dumpSkirt = (rot) => {
  const b = SHAPE_BUILDERS.tombFlat;
  const all = bake(() => withViewShear(0, () => withYaw(-rot, b)));
  return zsorted(lodFilter(all, 3)).map((f, i) => [i, f[2] ?? "", f[3] ?? null, String(f[0]).slice(0, 90)]);
};
...
await page.waitForFunction("!!window.__dumpSkirt");
for (const rot of [0]) {
  const rows = await page.evaluate((r) => window.__dumpSkirt(r), rot);
  console.log(`\n=== rot ${rot} : 면 ${rows.length}장 ===`);
  for (const [i, fill, key, d] of rows) console.log(String(i).padStart(3), (fill||"-").padEnd(9), String(key).padStart(8), d);
}
await browser.close();
process.exit(0);
