...
export const shapeMapTiles = (kind0: string): number => {
  ...
  // 시네마틱에서는 자원도 홀 기준 분수로 줄어든다(engine9 cineResTiles9) — 지도와 같은 자를 쓴다.
...
const irradSpr9 = (): HTMLCanvasElement => domSpr9("irrcloud", (c9, S9) => {
  ...
  /* ★ 둥글되 한 덩이가 아니라 **작은 가스 구름 몇이 합쳐진** 꼴(2026-09, 지적 셋: "좀더 둥글게" → "한 덩이가 아니라 아까처럼 작은
     가스 구름이 여러 개 합쳐져서 둥근 모양" → "개수가 과해 좀 줄여서 한 덩이들이 크게") — 열아홉은 알갱이 무늬로 읽혔다. 가운데 하나 +
     바깥 고리 일곱(0.4R · 반지름 0.36R)으로 줄이고 낱개를 키워 덩이마다 눈에 걸린다. 둘레는 일곱이 겹쳐 원에 가깝다. */
  puff9(R9, R9, R9 * 0.42, 0.6);
  for (let i9 = 0; i9 < 7; i9 += 1) {
    const an9 = (i9 / 7) * Math.PI * 2; const rd9 = 0.4 * R9;
    puff9(R9 + Math.cos(an9) * rd9, R9 + Math.sin(an9) * rd9, (0.34 + (i9 % 2) * 0.04) * R9, 0.62);
  }
}, 96);
  ...
  const tailD9 = st.muzzleLit || st.span || (st.trail && f.kind === "shot")
    ...
    if (st.puff) {
      ...
      /* ★ 간격의 바닥을 2 → 0.9px로(재요청: "더 자주 나오게") — 덩이를 4분의 1로
         ...
         ★ 바닥도 **간격과 같은 몫으로** 움직인다(2026-09, 요청: "지름 2배·빈도 반") —
           1.0 → 2.0. 아주 가까운 사거리에서는 이 바닥이 실제 간격을 지배하므로, 여기를
           안 올리면 그 자리에서만 빈도가 안 줄고 커진 덩이가 도로 겹친다. */
      const step9 = Math.max(2.0, st.w * tz9 * st.puff);
      ...
      for (let i9 = 0; i9 <= n9; i9 += 1) {
        ...
        /* 반지름 **평균 20% 확대**(요청) — 3.8 → 4.56. 들쭉날쭉(jit)도 같은 비로 키운다,
           안 그러면 덩이가 커진 만큼 낱개의 차이가 상대적으로 묻혀 자취가 고르게 읽힌다. */
        /* ★ **지름 2배 · 빈도 반**(2026-09, 요청: "미사일트레이서류의 연기 지름 2배로 키우고
           대신 빈도 반으로 낮추기") — 4.56 → 9.12(들쭉날쭉 0.72 → 1.44도 같은 비)이고
           간격(갈래표 puff 9.33 → 18.66)·바닥(step9 2.0)이 **같은 2배**다. 곧 이웃 덩이의
           겹침 비가 한 톨도 안 바뀌고 덩이만 굵고 성글어진다 — 위 ★ 의 '간격과 지름은 늘
           한 쌍' 규약 그대로다(한쪽만 2배면 한 줄기 띠이거나 점선이 된다).
           ★ **거기서 크기만 다시 1.4배**(2026-09, 요청: "빈도는 그대로인채로 크기만 1.4배") —
             9.12 → 12.768(들쭉날쭉 1.44 → 2.016). 이번엔 **간격을 일부러 안 건드렸다**:
             지름 25.5 vs 간격 18.66 이라 이웃끼리 27% 겹쳐, 성글던 구슬 줄이 도톰한
             밧줄로 읽힌다(그것이 '빈도는 그대로'의 뜻이다 — 위 한 쌍 규약의 예외다). */
        const r9 = st.w * tz9 * (12.768 + 2.016 * jit9);   // 타일 자(폰 보정, 핵탄두와 같은 지적)
...
const FX_NO_FLOOR = new Set<FxOp["kind"]>(["beam", "shot", "cage", "burst", "wound", "dom"]);
...
export const PLAYBACK_ZOOM_MAX = 16;   // 확대 사다리의 맨 위(요청: 1-2-4-8 → 16 칸 추가)
...
export default function ReplayMotionPlayer({
  ...
  /* ── 추적 카메라(요청) — 가장자리에 닿을 때만 다시 잡는다 ────────────────────────
     ...
     그 밖(가장자리 띠 TRACK_EDGE)으로 나가면 그때 다시 잡되 **한가운데로** 데려온다(요청).
     지금 그 띠는 0이라 **안전 상자가 곧 화면 전체**다 — 화면을 실제로 벗어날 때에만 움직인다.
   ...
   *  ★ **0이다**(요청: "외곽 20프로 말고 화면을 최소한이라도 벗어날 때로 완화") —
   *    안전 상자가 곧 화면 전체다. 0.2(안전 60%)·0.1(안전 80%)에서는 화면이 아직
   *    넉넉한데도 카메라가 먼저 움직여, 안 흔들리게 하려던 뜻이 반만 섰다. 이제는
