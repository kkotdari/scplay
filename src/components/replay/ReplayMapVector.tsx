...
export default function ReplayMapVector({
  ...
  useEffect(() => {
    ...
    MAPVEC_M9.bake += 1;
    /* 이 한 번이 몇 ms였나(위 MAPVEC_M9) — 1배·큰 맵에서 이 판이 메인 실마리를 초 단위로
       잡는지가 로딩의 갈림길이다. 합과 최악을 함께 든다(몇 번 굽는지는 bake가 센다). */
    { const bm9 = performance.now() - bt09; MAPVEC_M9.ms += bm9; if (bm9 > MAPVEC_M9.max) MAPVEC_M9.max = bm9; }
    ...
    if (painter) {
      painter.current = (z9, p9) => {
        ...
        /* hold를 걷었다(요청: "팬·드래그시 실시간으로 모델을 그릴 순 없나") — 손짓 중 절대 안 굽게 막아 두니, 여유
           (pad 1.45)를 넘어 끌면 새로 드러난 쪽이 빈 바탕으로 남았다. 이제 필요할 때는 굽는다: 벡터층이 맨 앞에서
           '이미 구운 창 안인가'를 보고 대개 곧장 빠져나가므로(bakedRef), 여유 안에서는 종전과 똑같이 안 굽는다. */
        paint(z9, p9, zq, 1.45, false);
      };
    }
