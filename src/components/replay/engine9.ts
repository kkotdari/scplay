...
export const MODEL_NORM: Record<string, number> = {
  arbiter: 1.658,  // 상자 상한(원한 배수 2.224)
  archon: 0.480,  // 재측정(model-norm)
  bc: 0.589,
  burrowhole: 0.735,
  carrier: 0.633,
  corsair: 1.218,  // 재측정(model-norm) — 위팔 0.8배 뒤
  darchon: 0.496,
  defiler: 0.740,   // 0.687 → ×1.2(요청: 디파일러 그리기 1.2배)
  devourer: 0.747,
  drone: 0.961,  // 재측정(model-norm)
  droneGas: 0.929,
  droneMin: 0.951,
  dship: 0.648,  // 포드 축소·안쪽 이동 뒤 재측정(model-norm)
  dtemp: 0.812,  // 재측정(model-norm) — 망토를 1.2배로 늘리고 들어 올린 뒤(잉크 폭 5.48 → 6.26)
  egg: 1.156,   // 정수리를 둥글게 한 뒤 model-norm 재측정
  fbat: 1.155,
  ghost: 1.489,  // 상자 상한(원한 배수 1.723)
  goliath: 0.652,   // 재측정(model-norm) — 미사일 포드 걷고 포신 하나로(옛 0.671)
  goon: 0.601,  // 재측정(model-norm)
  guardian: 0.672,  // 다리 옆으로 곧게 뒤 재측정(model-norm)
  gunner: 1.242,  // 팔 길이 고정 뒤 model-norm 재측정
  htemp: 1.077,  // 재측정(model-norm) — 왕관(살 다섯·보석)까지 반영
  hydra: 0.614,  // 머리장식 확대·머리 위 시작·길이 +20%·폭 −20%·마름모 허리 0.75 뒤 재측정(model-norm의 **top 모드** 표)
  inf: 1.430,  // 상자 상한(원한 배수 1.615)
  interceptor: 1.394,  // 재측정(model-norm)
  larva: 1.265,  // 상자 상한(원한 배수 1.466)
  lurker: 0.535,
  lurkeregg: 0.802,
  mine: 0.732,  // 다리를 걷은 뒤 재측정은 1.840이지만 몸 크기를 지키려 옛 값 1.007 유지 → ×0.8(요청: "벌처랑 마인 0.8배")
  muta: 0.688,
  mutacocoon: 1.690,  // 상자 상한(원한 배수 1.891)
  observer: 1.750,
  ovie: 0.764,  // 앞턱을 호로 바꾼 뒤 재측정(model-norm)
  probe: 1.589,  // 다리 두께면을 양쪽으로 고친 뒤 model-norm 재측정
  probeGas: 1.373,
  probeMin: 1.422,
  queen: 0.569,
  reaver: 0.777,  // 재측정(model-norm)
  scarab: 1.399,  // 상자 상한(원한 배수 1.591)
  scourge: 1.177,  // 상자 상한(원한 배수 1.327)
  scout: 0.761,  // 재측정(model-norm)
  scv: 0.692,  // 하완 1.2배·상완 원통 걷은 뒤 재측정(model-norm) — 옛 0.767
  scvGas: 0.779,
  scvMin: 0.788,
  shuttle: 0.722,  // 재측정(model-norm) — 뒷다리를 더 오므린 뒤
  ...
  tanksiegebody: 0.675,
  ultra: 0.326,
  valk: 0.613,   // 앞동체 −10% 뒤 재측정(model-norm) 0.840 → ×0.8(요청: 발키리 그리기 0.8배)
  vessel: 0.834,  // 방패 접힘 축·뾰족 위끝 뒤 재측정(model-norm)
  vulture: 0.604,   // 0.833 × 0.8(요청: "벌처랑 마인 0.8배")
  wraith: 0.810,  // 재측정(model-norm)
  zealot: 0.920,  // 투구·관(마루 6.36)·묶음 재작도 뒤 재측정(model-norm의 top 모드) · 옛 0.955
  zling: 0.699,
...
export const UNIT_SIZE_GLOBAL: number = 1.0;
/** 화면 크기의 유일한 입구(타일).
...
/* ★ +45 → **−30**(지적: "좌우도 더 왼쪽에서 보는 것 같고" · "원작보다 요잉이 더 많이
   된 것 같은데") — 부호가 보는 쪽을 가른다: +면 건물 앞면이 화면 왼-아래를 보므로
   **오른쪽에서** 본 그림이 되고, −면 앞면이 오른-아래를 봐 **왼쪽에서** 본 그림이 된다.
   크기는 45 → 30으로 줄여 덜 돌아간 채로 둔다. 이 한 값이 모든 건물의 기본 방향이고,
   총구 각(bunkerMuzzleOf)·앵커도 이 값에서 풀리므로 여기만 고치면 된다. */
export const BUILDING_BASE_YAW = -30;
...
export const BLD_NORM: Record<string, number> = {
  academy: 1.466,  // 치마형 받침으로 바꾼 뒤 bld-norm 재측정
  arch: 2.077,  // 판 0.9배 뒤 재측정 2.599 × 0.8(요청: 전체 그려지는 크기 0.8배)
  archives: 1.732,  // ×0.8(요청: 그려지는 크기 0.8배) · 옛 2.489
  armory: 1.209,
  assim: 1.502,  // 재작 뒤 재측정(bld-norm)
  cavern: 1.079,
  citadel: 1.518,  // 돛 두께 0.26·띠를 드럼/반구 따로 뒤 재측정(bld-norm 2.145) × 0.8(요청: 0.8배)
  cocoon: 2.041,
  coil: 1.333,
  comsat: 1.351,
  covert: 1.509,  // 사진 재작도 뒤 재측정(bld-norm)
  creep: 1.107,  // ×0.8(요청: 그리기 0.8배) · 발 셋 이식·요잉·촛대·촛대 가시 뒤 재측정 1.438 · 옛 1.232
  ctower: 1.650,
  ...
  cube: 1.209,   // 다리 안쪽·뒤쪽 앞으로 25%·기둥 20% 축소 뒤 재측정 0.998 × 1.2 · 옛 1.055
  cyber: 1.568,  // ×0.8(요청: 그려지는 크기 0.8배)
  diamond: 1.905,  // 상자 상한에 걸림
  dmound: 1.111,
  dome: 1.134,  // ×0.8(요청: 그려지는 크기 0.8배)
  ebay: 1.481,   // 다리 두 마디 20% 축소 뒤 재측정(잉크 폭이 좁아져 배수는 올라간다)
  evo: 1.219,  // ×0.8(요청: 그리기 0.8배) · 가운데 장기 경사·뒤 등걸 0.8 뒤 재측정 1.476 · 옛 1.232
  extract: 0.931,  // ×0.8(요청: 그리기 0.8배) · 창 2.5배(다섯) 뒤 재측정 1.066 · 옛 1.027
  factory: 1.573,  // 창·옆 홈·격납구·발판·굴뚝·쐐기 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  fleetbeacon: 1.654,  // ×0.8(요청: 그려지는 크기 0.8배)
  forge: 2.120,   // 돔 둘을 회전체로 바꾼 뒤 bld-norm 재측정
  gate: 2.004,  // 앞뒤 뿔탑 0.8배·발판에 붙인 뒤 재측정(bld-norm)
  geyser: 1.587,
  ...
  mineralb: 1.264,
  mineralc: 1.583,
  mshop: 1.571,  // −90° 요잉 뒤 재측정(bld-norm)
  nsilo: 1.325,
  nydus: 1.186,
  observatory: 1.864,  // 재측정 1.999 × 0.8(요청: 그려지는 크기 0.8배) — 바닥 관 제거·마름모 팔 뒤
  physlab: 1.510,  // 사진 재작도 뒤 재측정(bld-norm)
  plane: 1.254,   // 다리 낮춤·안테나 자 제외(boxSkip) 후 bld-norm 재측정
  pool: 1.217,  // ×0.8(요청: 그려지는 크기 0.8배) · 옛 1.449
  pyramidWide: 1.040,  // 재측정(bld-norm) — 기둥 5.5·원기둥 받침·표창 길이 원복 뒤
  queensnest: 1.145,   // 재측정(bld-norm) — 돔 5.6으로 키운 뒤(옛 1.148)
  refinery: 1.433,  // 아래판 뒤를 줄이고 뒤 드럼을 앞으로 당긴 뒤 bld-norm 재측정(1.394 →)
  robobay: 1.613,  // 재측정 1.909 × 0.8(요청: 그려지는 크기 0.8배) — 가운데 결정을 작은 반구로 바꾼 뒤
  sbattery: 1.945,  // 빨대 다리 뒤 재측정(bld-norm)
  scifac: 1.421,  // 재작도 + 삼중탑 제거·왼판 축소 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  spire: 1.552,  // 상자 상한에 걸림
  spore: 1.333,  // ×0.8(요청: 그리기 0.8배) · 가운데 생체 포신 뒤 재측정 1.669 · 옛 1.333
  sunken: 0.920,  // ×0.8(요청: 그리기 0.8배) · 왼쪽 큰 발(길이 0.8배) 뒤 재측정 1.286 · 옛 1.046
  sunkenfire: 1.248,
  tomb: 1.639,   // 옆 구리띠에 두께를 준 뒤 재측정 · 옛 1.534
  tombFlat: 1.779,   // 높이 1.1배·계단 2/3·그리기 크기 원복 뒤 재측정
  trapezoid: 2.789,
  tribunal: 1.681,  // ×0.8(요청: 그려지는 크기 0.8배)
  turret: 1.918,  // 포드를 팔자로 굴린 뒤 재측정(bld-norm) — 잉크 폭 9.05 → 8.98
  warpin: 2.196,
};
