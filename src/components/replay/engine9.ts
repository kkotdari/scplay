...
export const BLD_DRAW_TUNE: Record<string, number> = {
  ...
  pool: 1.2,
  /* ★ 2026-09, 요청: "캐번 1.2배 마운드 0.8배로 그리기"(1 을 자로 말한 값 — 이 표의 그때 값이 둘 다
     1.2 였으므로 **거기에 곱한다**). 평면에서 그리는 크기는 `BLD_DRAW_K × tune` 이라 tune 에 선형이다
     (`bldDrawK9` 는 CINE9 ≤ 0 이면 tune 을 안 본다 — 시네마틱에서만 안에서 나누고 밖에서 곱해 지워진다). */
  dmound: 1.2 * 0.8, cavern: 1.2 * 1.2,                                    // 캐번 1.4 → 1.2 → ×1.2 · 마운드 1.2 → ×0.8
};
...
export const FX_BEAM: Record<string, {
  ...
  missile: { dur: 0.28, w: 0.15, l: 24, puff: 9.33, smoke: "#e2ecfa", smokeEdge: "#8fc6ff", smokeFade: 0.95, warhead: "#a8b0bc", g: [[0, "rgba(220,232,248,1)"], [0.1, "rgba(196,218,244,0.94)"], [0.32, "rgba(176,200,232,0.6)"], [0.68, "rgba(154,172,196,0.32)"], [1, "rgba(134,150,174,0)"]], glow: "rgba(168,198,238,0.5)" },
  ...
  missileG: { dur: 0.28, w: 0.15, l: 24, puff: 9.33, smoke: "#e2ecfa", smokeEdge: "#8fc6ff", smokeFade: 0.95, warhead: "#c9a132", g: [[0, "rgba(220,232,248,1)"], [0.1, "rgba(196,218,244,0.94)"], [0.32, "rgba(176,200,232,0.6)"], [0.68, "rgba(154,172,196,0.32)"], [1, "rgba(134,150,174,0)"]], glow: "rgba(168,198,238,0.5)" },
