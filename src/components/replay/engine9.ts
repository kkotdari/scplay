...
export const FX_BEAM: Record<string, {
  ...
  /* ★ **연기까지 금색**이다(지적: "스카우트 트레이스 탄두는 금색이지만 연기는 테란과
     같은 색") — 앞 값은 속 #f4eddc(244,237,220)라 사실상 흰색이었고, 테 #ffe6a8도
     반지름의 바깥 한 겹에만 깔린다. 그 크기(반지름 몇 px)에서 두 색은 테란 미사일의
     흰빛·하늘빛과 화면에서 안 갈렸다 — 금은 탄두 삼각 하나에만 남아 있었다.
     속을 노란 기 도는 크림(#fae7b0)으로, 테를 제 금색(#dfae3c)으로 내린다. 배기
     그러데이션도 같은 계열로 옮겨, 자취 전체가 한 색 결로 읽힌다. 불꽃으로 안 보이게
     붉은 기는 안 넣는다(주황이 섞이면 곧 화염이다) — 노랑에서 황금으로만 간다. */
  missileG: { dur: 0.28, w: 0.15, l: 24, puff: 8.4, smoke: "#fae7b0", smokeEdge: "#dfae3c", smokeFade: 0.95, warhead: "#c9a132", g: [[0, "rgba(255,241,196,1)"], [0.1, "rgba(250,226,152,0.94)"], [0.32, "rgba(232,199,104,0.6)"], [0.68, "rgba(198,165,74,0.32)"], [1, "rgba(164,136,62,0)"]], glow: "rgba(230,192,88,0.55)" },
