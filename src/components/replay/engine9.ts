...
const MUZZLE_HAND9: Record<string, [number, number, number]> = {
  ...
  tank: [0.42, 3.02, 3.75], tanksiege: [0, 3.73, 5.07],
...
export const MODEL_NORM: Record<string, number> = {
  arbiter: 1.760,  // 상자 상한(원한 배수 2.224) · 날개 뒤끝 안으로 휘고 안쪽 평면 뒤 ×1.0435(2.229 → 2.326)
  archon: 0.480,  // 재측정(model-norm)
  bc: 0.630,   // 날개 스팬 20% 축소·동체 20% 축소 뒤 재측정 비(0.619 → 0.662)로 되올림 · 로켓 십자 깃(0.665) · 추진체 모음(0.656) · 미사일 뒤로(0.635) · 화살촉(0.632) · 깃 폭 반(0.653) · 관 0.11 뒤 ×1.0046(0.653 → 0.656) · 옛 0.596
  burrowhole: 0.732,
  carrier: 0.644,
  ...
  droneGas: 0.933,
  droneMin: 0.958,
  dship: 0.661,  // 포드 축소·안쪽 이동 뒤 재측정(model-norm)
  dtemp: 0.847,  // 재측정(model-norm) — 망토를 1.2배로 늘리고 들어 올린 뒤(잉크 폭 5.48 → 6.26)
  ...
  probeMin: 1.475,
  queen: 0.585,
  reaver: 0.803,  // 재측정(model-norm)
  scarab: 1.451,  // 상자 상한(원한 배수 1.591)
  scourge: 1.272,  // 상자 상한(원한 배수 1.327) · 재작도 1.174 · 팔 바깥 1.095 · 피칭 1.133(1.159 → 1.199) · 손톱 안쪽 뒤 ×1.1226(1.199 → 1.346)
  ...
  tanksiege: 0.695,
  tanksiegebody: 0.687,
  ultra: 0.369,   // 팔뚝을 걷고 낫을 몸에 붙인 뒤 ×1.1134(0.344 → 0.383)
  ...
  zealot: 0.968,  // 투구·관(마루 6.36)·묶음 재작도 뒤 재측정(model-norm의 top 모드) · 옛 0.955
  zling: 0.727,
