    ...
    if (fxName9 && NO_BEAM_FX.has(fxName9)) {
      ...
      if (beamLen > 1 && MUZZLE_BURST_FX9.has(fxName9)) {
        ...
        if (uS9 < 1) {
          ...
          put9(mzx9, mzy9, fxPx * 0.38, fxName9, 0.12 + uS9 * 0.88, beamDeg);
          ...
          if (sp9 <= 1) put9(tvx9, tvy9, fxPx * (fxName9 === "siege" ? 1.5 : 1.1), "tankboom", sp9);
        }
      } else if (beamLen > 1 && fxName9 !== "cannon") {
        /* ⚠ **일반(탱크) 모드의 전차포는 아무것도 안 그린다**(2026-09, 요청: "일반 시즈모드에서는
           총구와 탄착지 폭발 효과는 제거") — 포구 불꽃·에너지포·착탄 스플래시 셋은 **시즈 모드의
           것**이었는데(그 요청의 "시즈탱크 트레이서"), 갈래표에 `cannon` 까지 얹어 두어 일반 모드의
           작은 90mm 포까지 큰 폭발을 냈다. 일반 모드의 사격은 **포탑 반동 컷**이 말한다. */
        const [tfx9, tfy9] = posFrac(foe.bx, foe.by);
