...
export const HOVER_RISE_K: Record<string, number> = {
  ...
  // 다시 반으로(재요청: "부양 지상유닛 5종 떠있는 높이 반으로") — 0.02/0.022/0.025 → 0.01/0.011/0.0125.
  Probe: 0.01, SCV: 0.01, Drone: 0.01,
  "High Templar": 0.011,
...
export function atkCutOf(
  ...
  /* ★ 고스트 — 총을 **든 채** 앞뒤로 퉁퉁 반동한다(2026-09, 지적: "고스트 공격 모션은
     총을 들고 퉁퉁 앞뒤 반동 주는 거여야 하고, 총을 꺼내는 건 기타 액션으로 가야 해") ──
     옛 한 바퀴는 '반쯤 → 겨눔 → 반쯤 → 등에 멤'이라 **쏠 때마다 총을 넣었다 꺼냈다** 했다.
...
export const ATTACK_FX: Record<string, string> = {
  ...
  Vulture: "frag",
  Battlecruiser: "laserBC", "Siege Tank": "cannon", "Siege Tank (Tank Mode)": "cannon",
  ...
  Interceptor: "gun",
  /* ★ **일꾼도 제 트레이서가 있다**(2026-09, 요청: "일꾼들 공격 트레이서 추가 — scv 스파크
     드론 침뱉기 프로브 지지기") — 셋 다 때리는 유닛인데 표에 없어 아무것도 안 그렸다.
     SCV 는 융합 절단기라 총구 스파크(spark) · 드론은 산을 뱉고(spit) · 프로브는 입자
     빔으로 지진다(weld — 짧게 지지직거리는 푸른 아크). */
  SCV: "spark", Drone: "spit", Probe: "weld",
 ...
 *  (대공은 미사일 발사관, 지상은 연사 기관포·버스트 레이저). 표(ATTACK_FX)는 '이 유닛의 무기'를
 *  한 줄로 말하는 자리라 그 셋을 한 줄로 못 적는다.
 *  ⚠ 이 규칙이 **그리는 자리에만** 있으면 도록은 못 본다 — 지도와 도록이 같은 수를 보게
 *    여기 한 자리에 적는다(CLAUDE.md '도록의 칸' 의 ⚠ "값은 지도의 값을 그대로 쓴다").
 *  · 레이스의 지상만 표로 물러난다(제 laser) · 골리앗·스카우트의 지상은 총구 스파크다.
 *  · 대공 탄두는 스카우트만 금(missileG)이고 나머지는 은(missile)이다. */
export function attackFxOf9(unitName: string, air: boolean): string | undefined {
  const dual9 = unitName === "Wraith" || unitName === "Goliath" || unitName === "Scout";
  if (dual9 && air) return unitName === "Scout" ? "missileG" : "missile";
  if (dual9 && unitName !== "Wraith") return "spark";
  return ATTACK_FX[unitName];
}
...
export const MUZZLE_BURST_FX9 = new Set(["siege"]);
/** ★★ **탄만 나는 갈래**(2026-09, 지적: "트레이서가 안 나가") — 일반 탱크 모드(`cannon`)는
 *  포구 불꽃도 착탄 폭발도 안 내지만(요청: "일반 시즈모드에서는 총구와 탄착지 폭발 효과는
 *  제거") **날아가는 포탄은 그대로 있어야 한다**. 앞선 손질이 그 요청을 '셋 다 제거'로 읽어
 *  탱크 모드가 한 톨도 안 그렸다 — 요청이 이름 붙여 뺀 것은 **총구**와 **탄착지 폭발** 둘이다.
 *  그래서 이 갈래는 위 셋 중 ②(에너지포)만 낸다. */
export const SHELL_ONLY_FX9 = new Set(["cannon"]);
...
export const PROJECTILE_FX = new Set([
  "spine", "glave", "acid", "venom", "missile", "missileG", "plasma", "cannon", "siege", "frag", "spit", "yamato",
  // 레이저(레이스 지상·배틀크루저) — 짧은 광탄이 날아가 꽂힌다(그 표의 ★).
  "laser", "laserBC",
]);
...
/* z는 모델 z 0.8배 손질(scripts/model-z-scale.mjs)과 함께 ×0.8 했다 — 총구표(muzzleTable.gen)는 빌더 표식에서 다시 뽑힌다. */
const MUZZLE_HAND9: Record<string, [number, number, number]> = {
  gunner: [0.15, 2.7, 2.68], ghost: [0.4, 3.6, 2.736], fbat: [0.52, 3.4, 2.52],
  inf: [1.22, 2.05, 2.4],
  ...
  // 포탑 0.8배(turretScaled9: xyz×0.8, z +0.57) 뒤: 탱크 (0.42, 3.0, 3.2) → (0.34, 2.4, 3.13) · 시즈 (0, 3.7, 5.1) → (0, 2.96, 4.65).
  // 포탑(원판 제외) 1.4배 뒤: 탱크 끝 (0.42·1.4, 3.0·1.4, 3.1+0.98) → ×0.8 + 0.57 = (0.47, 3.36, 3.83) · 시즈 기울인 끝 (0, 5.19, 5.91) → (0, 4.15, 5.29).
  // 포신 0.9배 더(BX9 1.26) 뒤: 탱크 (0.53, 3.78, 3.98)×0.8+0.57z = (0.42, 3.02, 3.75) · 시즈 기울인 끝 (0, 4.67, 5.63) → (0, 3.73, 5.07).
  tank: [0.42, 3.02, 3], tanksiege: [0, 3.73, 4.056],
  /* 골리앗 [1.4, 2.2, 2.72] → [0, 2.9, 4.496] — 옛 값은 지금 모델에서 **무릎 높이**(z 3.4)의
     허공이었다. 총열은 z 5.62에 있고, 미사일은 좌우 두 줄기로 갈라져 나가므로(lanes9)
     앵커는 몸 한가운데여야 두 발이 양 포드에 하나씩 선다 — x를 0으로 옮긴 까닭이다. */
  goliath: [0, 2.9, 4.12],   // 포신 하나·낮춤(z 5.62 → 5.15)
  vulture: [0, 3.4, 2.08], wraith: [0, 3.1, 3.104], bc: [0, 4.6, 3.04],
  /* 발키리는 **몸 가운데**에서(지적: "나오는 위치가 안맞고") — x 0.9는 오른쪽 발사관
     하나를 짚은 값이라, 어느 요잉에서는 미사일이 몸 옆 허공에서 났다. 발키리는 좌우
     발사관이 함께 뿜으므로 가운데가 두 줄기의 대표다. */
  valk: [0, 3.4, 4.48],   // 부리 끝
  hydra: [0, 2.6, 3.36], lurker: [0, 3, 1.76], muta: [0, 3, 2.4], queen: [0, 3, 2.4],
  guardian: [0, 3.2, 2.24], devourer: [0, 3.2, 2.24], ultra: [0, 3.6, 2.72],
  goon: [0, 3.2, 2.88], zealot: [0.8, 2.4, 2.4],
  /* ★ 아콘은 **앞으로 안 내민다**(지적: "플라즈마의 시작부가 아콘 쪽이 아니고 표적에
     ...
     표적이 어디로 돌든 안 움직인다. */
  archon: [0, 0, 3.52], reaver: [0, 3.6, 1.92],
...
const BLD_MUZZLE_HAND9: Record<string, [number, number, number]> = {
  coil: [0, 0, 5.76], sunken: [0.35, 0.15, 2.96], spore: [-0.35, 2.6, 2],
  // 터렛 z 8.2 → 10.8(요청: 밑받침 1.5배 + 포드 여유) — 새 포드 꼭대기 pvt(1.9, 5)다.
  turret: [2.2, 0.6, 8.64], tombFlat: [0, 2.7, 0.96],
};
...
export const MODEL_NORM: Record<string, number> = {
  /* ★ 재측정 2.307(model-norm top · 상한 4.27)(2026-09, 지적: "아비터는 왜 맨날 작게 그려지는 거야") — 옛 값 1.760 은
     날개를 반으로 줄이고 축을 ±0.95 → ±0.58 로 당기기 **전**의 잉크(≈2.95)에서 온 값이라, 지금 잉크 2.25 에 그대로 걸면
     목표(5.2)의 76% 로 서서 늘 작았다. 코드모드 되돌림 때 '옛 값 복원'으로 적혀 낡은 채 남은 자리다. */
  arbiter: 1.007,   // 원 좌표를 ARB_K9(2.307)로 키우고 배율을 1 로(2026-09, 요청: "아비터 원 좌표를 키우고 배율을 낮춰") — model-norm 재측정값으로 갈아 둔다
  archon: 0.480,  // 재측정(model-norm)
  bc: 0.630,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  burrowhole: 0.732,
  carrier: 0.605,   // 잎을 더 볼록하게 한 몫까지(model-norm top 0.636 → 0.619)
  corsair: 1.251,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  darchon: 0.496,
  defiler: 0.870,   // ×(0.753/0.745) 여섯 다리 무릎(안쪽 + 앞·뒤로 꺾기 — 잉크 폭이 준 만큼, 몸 크기는 그대로) · 0.687 → ×1.2(요청: 디파일러 그리기 1.2배) → ×(0.700/0.649) 다리 0.8배·뒷다리 뒤로 → ×(0.720/0.700) 세 쌍 손질(실측 0.700→0.720) → ×(0.745/0.720) 뒷다리 과감히 뒤로·0.9배(실측 0.720→0.745)
  devourer: 0.780,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  ...
  goon: 0.619,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  guardian: 0.675,  // 다리 옆으로 곧게 뒤 재측정(model-norm) · … · 다리 끝 뾰족(0.672, 0.683 → 0.701) · 날개껍질 내리고 척추 앞 반 뒤 ×1.0043(0.701 → 0.704)
  gunner: 1.302,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  htemp: 1.125,   // 망토를 뒤로 젖힌 뒤 재측정(model-norm top 1.170 → 1.155)
  hydra: 0.646 * 1.08,  // ×1.2 → ×0.9(2026-09, 요청: "히드라 그려지는 크기 10프로 줄이기" — 곱한 값 · 재측정 아님) · 머리장식 확대·머리 위 시작·길이 +20%·폭 −20%·마름모 허리 0.75 뒤 재측정(model-norm의 **top 모드** 표)
  inf: 1.515,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  interceptor: 1.490,  // 셋 다 방추로 · 가운데를 줄인 몫(model-norm top 1.472 → 1.555)
  larva: 1.268,  // 상자 상한(원한 배수 1.466)
  lurker: 0.545,
  lurkeregg: 0.817,
  mine: 0.744,  // 다리를 걷은 뒤 재측정은 1.840이지만 몸 크기를 지키려 옛 값 1.007 유지 → ×0.8(요청: "벌처랑 마인 0.8배")
  muta: 0.710,
  ...
  /* ★ 시즈탱크 넷은 **차체 하나의 값으로 못 박는다**(지적: "정규화 시 포신 튀어나온
     ...
     여기는 손으로 지킨다. */
  /* 한때 ×1.2(0.868)로 키웠다가 되돌림(요청: "탱크 다시 1배로") — 포신·버팀다리는 NORM_PAIR로 같은 배수를 물려받는다.
  ...
  tank: 0.687,   // 넷은 한 값(위 ★) — 합본의 재측정 0.701 도 덮지 않는다
  tankbody: 0.687,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  /* ⚠⚠ **여기를 재측정 값으로 덮지 마라 — 위 ★ 의 못 박기가 이긴다**(2026-09, 지적: "탱크
     시즈모드시 크기가 작아지고 색이 어두워지는데 왜 그러지 모델이 다른가") — 포신을 뒤로
     돌린 뒤 model-norm 을 그대로 실어(0.695 → 0.602 · 0.687 → 0.628) 넷을 한 값으로 두던
     규약이 깨졌다. 시즈의 잉크 상자에는 **네 귀의 버팀다리와 뒤로 뻗은 포신**이 들어가므로
     그 상자에 맞추면 **같은 부품인 궤도·차체가 8.6% 쪼그라든다** — 화면에서는 시즈를 걸
     때마다 탱크가 작아지는 것으로 보이고(실측), 그만큼 결·광택의 배율 문턱(k)도 함께
     내려가 **덜 반짝여 어두워 보인다**. 모델 자체의 밝기는 같다(실측: 여덟 각 평균 138.9
     vs 140.6 — 시즈가 오히려 조금 밝다).
     짐 든 일꾼의 규약(NORM_PAIR 의 ★)과 **같은 자리**다: 자세·부속으로 달라진 잉크는
     재측정으로 덮지 않는다. 상한(축)은 1.14 라 0.687 로도 안 잘린다(model-norm 실측). */
  tanksiege: 0.687,
  tanksiegebody: 0.687,
  ultra: 0.329,   // 켄타우로스 상체 뒤 재측정(요청: "울트라 몸크기 축소" — 0.369 → 0.329 · 옛: 모델 z 손질 재측정 되돌림)
  ...
  zealot: 0.968,   // 모델 z 손질 재측정 되돌림(2D 기하 복원)
  zling: 0.727,
...
/* ★ 1.12 → **1.0**(요청: "모든 유닛 크기 배율 1.0으로") — 건물 대비가 작아 보이던 것을
   무리 전체에 걸어 키우던 손잡이인데, 그 사이 건물 쪽 자(BLD_FILL_TARGET·BLD_NORM)를
   여러 차례 손보며 기준이 달라졌다. 1로 두면 화면 크기는 원작 상자(units.dat) 비율에
   종류별 보정(UNIT_SIZE_TUNE)만 곱한 값이 된다 — 손잡이는 그대로 남아 있으니 다시
   키울 일이 있으면 이 한 값만 올린다. */
/* 1.0 → **1.2**(요청: "모든 유닛 그리기 1.2배 글로벌 적용"). 크기를 키우는 자리는 **여기**다 —
   모델 공간 배수(MODEL_NORM)에 곱하면 16-상자를 넘겨 마흔아홉 종이 판에서 잘린다(실측:
   그 배수 × 1.2가 종류마다의 상한을 넘는다). 이 손잡이는 화면 타일 수를 키우므로 판이
   그만큼 크게 구워질 뿐, 상자를 안 넘는다. 종류 사이의 비도 한 톨도 안 바뀐다. */
export const UNIT_SIZE_GLOBAL: number = 1.2;
/** 화면 크기의 유일한 입구(타일).
...
/** ⚠ **홀은 지금 크기를 지킨다**(재요청: "홀은 현재 크기가 유지돼야 하고 다른 모델들은 그 비에
 *  맞춰서 조정돼야 해 — 보통 크기가 다 줄겠지") — 자를 홀로 잡았다고 홀을 키우면 전장이 통째로
 *  커져 배율 감각이 바뀐다. 그래서 값은 **홀의 지금 폭**(발자국 4 × BLD_DRAW_K)이고, 다른
 *  모델은 그 아래로 **줄기만** 한다. 곧 시네마틱을 켜도 홀만은 한 화소도 안 움직인다. */
const CINE_HALL_T9 = 4 * 1.2;   // = 발자국 4 × BLD_DRAW_K(1.2). 그 상수는 아래에 서므로 값으로 적는다.
let CINE9 = 0;
...
/* ★ 부호가 보는 쪽을 가른다: +면 건물 앞면이 화면 왼-아래를 보므로 **오른쪽에서** 본
   그림이 되고, −면 앞면이 오른-아래를 봐 **왼쪽에서** 본 그림이 된다. 한때 −30으로
   돌려 두었는데("좌우도 더 왼쪽에서 보는 것 같고"), 원작 렌더 그림의 지붕 벤트 상자
   네모를 재 보니 요잉은 **+44~50도**였다 — 앞면이 화면 왼-아래를 보는 쪽, 곧 +45다.
   눈짐작과 어긋나므로 실측을 따른다. 이 한 값이 모든 건물의 기본 방향이고 총구 각
   (bunkerMuzzleOf)·앵커도 여기서 풀리므로 이 줄만 고치면 된다. */
export const BUILDING_BASE_YAW = BLD_YAW9;   // 45 → 55(요청: "수평 시점은 45도 이상")
...
/** 도록의 **방위 눈금**을 그 갈래의 기준각으로 옮긴다(요청: "도록은 45도 단위인데 건물은
 *  40도라 어떡하지") ────────────────────────────────────────────────────────────────
 *  건물은 지도에서 각이 하나뿐(BUILDING_BASE_YAW)이므로, 도록의 네·여덟 방위는 그 각을
 *  90도(혹은 45도)씩 돌린 것이라야 "도록에서 본 그 면"이 지도에서 그대로 나온다. 45 눈금을
 *  그대로 쓰면 건물만 5도 어긋나 앞면이 지도와 미묘하게 다른 각으로 선다. 유닛은 제 방향이
 *  자유로우니 45 눈금 그대로다. 옮긴 각은 yawBucket9가 죄지 않고 지난다(그 함수의 ★). */
export const galleryYawOf = (deg: number, group?: string): number =>
  (group === "건물" ? (((deg - 45 + BUILDING_BASE_YAW) % 360) + 360) % 360 : deg);
...
export const BLD_DRAW_TUNE: Record<string, number> = {
  ...
  tombFlat: 1.1,                                                           // 벙커 1.2 → 1.0 → 1.1(2026-09, 요청: "벙커 1.0배" → "벙커 1.1배" — 곱이 아니라 값 자체)
  ...
  turret: 1.0,                                                             // 1.2 → 1.0 복구(요청) — 공통 1.2배만 탄다
  pool: 1.2,
  ...
  dmound: 1.0, cavern: 1.4,                                                // 마운드는 공통 1.2배만 탄다(터렛과 같은 자리)
};
...
export const BLD_NORM: Record<string, number> = {
  ...
  assim: 1.749,  // 굴뚝 위 옛 정지 김(세 켜 타원)을 걷은 뒤 재측정(bld-norm · 1.664 →) — 덩이만 남아 잉크가 좁아졌다
  cavern: 1.082,
  ...
  cube: 1.089,   // 다리 안쪽·뒤쪽 앞으로 25%·기둥 20% 축소 뒤 재측정 0.998 × 1.2 · 옛 1.055
  cyber: 1.578,  // ×0.8(요청: 그려지는 크기 0.8배)
  diamond: 1.905,  // 상자 상한에 걸림
  dmound: 1.111,
  dome: 1.132,  // 패임을 진짜 그릇으로 판 뒤 재측정(1.415) × 0.8(요청: 그려지는 크기 0.8배)
  ebay: 1.443,   // 다리 두 마디 20% 축소 뒤 재측정(잉크 폭이 좁아져 배수는 올라간다)
  evo: 1.181,  // ×0.8(요청: 그리기 0.8배) · 가운데 장기 경사·뒤 등걸 0.8 뒤 재측정 1.476 · 옛 1.232
  extract: 0.853,  // ×0.8(요청: 그리기 0.8배) · 창 2.5배(다섯) 뒤 재측정 1.066 · 옛 1.027
  factory: 1.557,  // 옥상 골·톱니 두 줄 · 허리 7.8 → 7.4 뒤 재측정(bld-norm)
  ...
  mineralb: 1.264,
  mineralc: 1.583,
  mshop: 1.537,  // −90° 요잉·해저드 자리 손질 뒤 재측정(bld-norm)
  nsilo: 1.379,   // 경고 판·창 띠 손질 뒤 재측정(bld-norm)
  nydus: 1.184,
  observatory: 1.599,  // 재측정 1.999 × 0.8(요청: 그려지는 크기 0.8배) — 바닥 관 제거·마름모 팔 뒤
  physlab: 1.467,  // 사진 재작도 뒤 재측정(bld-norm)
  plane: 1.237,   // 다리 낮춤·안테나 자 제외(boxSkip) 후 bld-norm 재측정 · 다리를 4.55 → 4.15 로 들인 뒤 재측정(1.226 → 1.237)
  ...
  spore: 1.335,  // ×0.8(요청: 그리기 0.8배) · 가운데 생체 포신 뒤 재측정 1.669 · 옛 1.333
  sunken: 1.030,  // ×0.8(요청: 그리기 0.8배) · 왼쪽 큰 발(길이 0.8배) 뒤 재측정 1.286 · 옛 1.046
  sunkenfire: 1.391,   // 혓바닥 아치 1.394(호 2.0·꼭대기 16) · 높이 2/3(10.7)·호 3.0으로 ×0.9977(1.286 → 1.283)
  tomb: 1.577,   // 옆 구리띠에 두께를 준 뒤 재측정 · 옛 1.534
  tombFlat: 1.724,   // 높이 1.1배·계단 2/3·그리기 크기 원복 뒤 재측정
  trapezoid: 2.488,
  tribunal: 1.826,  // −45도로 돌려 세운 뒤 재측정 2.282 × 0.8(요청: 그려지는 크기 0.8배)
  turret: 2.210,  // 포드 누름 차례를 고쳐(직사각형 복원) 키가 0.8배 준 몫 재측정(bld-norm)
  warpin: 2.196,
};
...
/** 공사 단계 수 — 3 → 5 → **10**(요청: "5단계가 아니라 10단계로 늘려도 되겠지").
 *  부품이 많은 건물일수록 칸이 적으면 한 칸에 몇 분의 일이 통째로 솟아 '자라는' 대신
 *  '툭 나타나는' 것으로 보인다. 낯 단위로 세우게 된 지금은 칸이 많을수록 곱게 자란다.
 *  ⚠ 값이 메시 열쇠(b:종류:단계:…)에 드는 자라, 짓는 중인 건물 한 채가 쥘 수 있는 벌 수도
 *    이만큼이다 — 다 지으면 그 열쇠는 안 쓰이고 LRU 가 걷어 간다. */
export const BUILD_STAGES = 10;
...
export type FxOp = {
  ...
  /** shot: 진행률 0~1. */
  u?: number;
  /** beam/shot: **좌우 한 쌍의 한 발**인가(2026-09, 요청: "한 쌍 트레이서의 경우 타겟 쪽은 타겟 중심 한 점을
   *  향해 가야 함 · 미사일이 타겟에 가까워질수록 좀 휘어야") — 붓이 나는 길의 조종점을 더 앞(0.72)에 두어
   *  두 발이 제 발사관에서 **나란히** 나가다 표적 가까이에서 한 점으로 모인다. */
  twin?: boolean;
...
export const FX_BEAM: Record<string, {
  ...
  span?: boolean;
  /** ★ **길이가 고정인가**(2026-09, 요청: "파뱃 트레이서는 길이 고정이라 타겟 거리와 무관하게 원래 길이로
   *  나와야 해 — 방향은 타겟 쪽이되 길이는 고정") — 화염방사기는 닿는 무기가 아니라 **뿜는** 무기라 줄기의
   *  길이(l)가 곧 그 그림이다. 참이면 붓이 표적 거리(reach)로 안 죄고 늘 l 만큼 뻗는다. 방향은 표적 쪽이되
   *  두 줄기는 **나란히** 간다(한 점으로 안 모은다 — 요청: "단 파뱃은 제외"). */
  fixed?: boolean;
  ...
  zig?: number;
  /** 갈지자 진폭 — 길이 비(zigAmp, 기본 0.16)와 굵기 배(zigW, 기본 2.4) 중 작은 쪽. 아콘 번개는 가늘어진 굵기와 무관하게 크게 꺾인다. */
  zigAmp?: number; zigW?: number;
  ...
  glowEnd?: string;
  /** ★ **원뿔 줄기** — [뿌리 폭 배수, 앞 끝 폭 배수](w 에 곱한다). 주면 굵기 하나짜리 획이
   *  아니라 뿌리에서 좁고 앞으로 갈수록 넓어지는 **채운 꼴**로 그린다(요청: "파뱃 트레이서
   *  시작점은 좀 좁고 앞으로 가면서 넓어지는 형태여야 함 · 야구 방망이 느낌").
   *  뿜는 것(화염방사)은 노즐에서 좁게 나와 퍼지는데, 같은 굵기의 획은 그냥 막대로 읽힌다.
   *  양 끝은 둥글게 닫는다 — 그래서 방망이 꼴이 된다. */
  cone?: readonly [number, number];
  /** ★ **뭉게뭉게 연기 덩이**(2026-09, 요청: "파뱃 화염방사 연기 자연스럽게(반투명 회색 연기
   *  뭉게뭉게로) … 앞쪽 자연스럽게 호 형태로 처리" · "디바우러 트레이서도 연기 느낌 나게") —
   *  줄기 위에 반투명 원을 여럿 흩어 얹는다. 굵기 하나로 그은 줄기는 어디까지나 **매끈한 띠**라
   *  연기로 안 읽힌다. 덩이는 앞으로 갈수록 커지고 옆으로 더 흩어져 저절로 호를 그린다. */
  billow?: number;
  /** 덩이의 색(알파는 smokeA 가 따로 든다) · 덩이가 시작되는 줄기의 몫(0~1) · 크기 배수 · 짙기. */
  billowCol?: string; billowFrom?: number; billowK?: number; billowA?: number;
  /** 열기 글로우 — 줄기 앞쪽에 얹는 둥근 번짐의 속색(요청: "열기 느껴지는 글로우 추가"). */
  heat?: string;
}> = {
  ...
  /* ★ 화염방사(파이어뱃) — **크고 화려하게**(지적: "지금 너무 소박하고 불 같지 않음").
     불은 한 색의 띠가 아니라 **속이 하얗고 밖으로 갈수록 붉어지다 검은 연기로 끝나는 것**
  ...
  // 원뿔(요청) — 뿌리 0.45배 · 앞 끝 2.4배. 노즐에서 좁게 나와 앞으로 퍼지는 방망이 꼴이다.
  /* ★ **불방망이지 손전등이 아니다**(2026-09, 지적: "파뱃 트레이서는 저렇게 손전등 불빛
     느낌이 아니라 불방망이 느낌이야 — 훨씬 더 길고 폭은 좁아야 해, 연기도 있어야 하고") ──
     옛 값은 길이 5.0 에 끝 폭이 뿌리의 **5.3배**(cone 0.45 → 2.4)라, 짧고 끝이 확 벌어지는
     **빛 원뿔**이었다. 화염방사기의 불은 그 반대다: 뿜은 기름이 한 줄기로 멀리 나가며 조금만
     퍼지고, 끝에서 **그을음**이 된다.
       · 길이 5.0 → **9.5**(거의 두 배) · 폭 자 2.0 → **1.05** · 벌어짐 2.4 → **1.5**
         (끝 반폭이 2.4·zoom → 0.79·zoom — 세 배 좁다).
       · 그러데이션 끝에 **연기**를 단다: 흰불 → 주황 → 벌건 숯 → **잿빛 연기**가 사그라진다.
         (0 쪽이 총구다 — muzzleLit 갈래라 뿌리가 밝다.)
       · 번짐 켜(glowW)도 함께 좁힌다 — 안 좁히면 줄기보다 후광이 굵어 도로 원뿔이 된다. */
  /* 연기·열기는 위 smoke·heat 가 낸다(요청) — 앞쪽 55% 에 회색 덩이 아홉이 뭉게뭉게 얹히고,
     그 뒤로 둥근 열기 번짐이 깔린다. 그러데이션 끝의 회색 꼬리만으로는 '흐려지는 띠'였다. */
  /* l 9.5 → 8.5(2026-09, 지적: "파뱃 트레이서 길이 더 길어 사정거리 생각해서") — 사거리 1타일 + 표적 반지름 안쪽(1.06타일). */
  flame: { muzzleLit: true, fixed: true, dur: 0.34, w: 1.05, l: 8.5, cone: [0.5, 1.5], glow: "rgba(255,146,48,0.5)", glowW: 1.55, glowA: 0.5,
    billow: 9, billowFrom: 0.45, billowK: 1.15, billowA: 0.26, billowCol: "rgba(156,150,146,1)",
    heat: "rgba(255,168,72,0.55)", g: [[0, "rgba(255,252,236,0.98)"], [0.07, "rgba(255,240,176,0.98)"], [0.2, "rgba(255,190,72,0.95)"], [0.42, "rgba(255,116,34,0.88)"], [0.62, "rgba(214,80,30,0.7)"], [0.78, "rgba(152,114,100,0.46)"], [0.9, "rgba(128,120,114,0.26)"], [1, "rgba(118,112,108,0)"]] },
  ...
  glave: { dur: 0.24, w: 2.85, l: 1.9, g: [[0, "#6b4732"], [1, "#6b4732"]], tri: true, glow: "rgba(120,84,58,0.6)" },
  ...
  // missile·missileG w 0.1 → 0.15 · puff 5.6 → 8.4(요청: 미사일류 1.5배 — 간격은 지름과 한 쌍)
  /* ★ 연기 덩이는 **크고 성기게**(2026-09, 요청: "미사일류 연기 크기 반지름 평균 20프로 확대해 주고
     등장 빈도는 10프로 줄이기") — 간격(puff) 8.4 → 9.33 이면 같은 길이에 덩이가 0.9배로 선다.
     반지름은 붓이 든다(그 자리의 4.56). 둘은 늘 한 쌍이다 — 반지름만 키우면 덩이가 서로 파묻혀
     도로 한 줄기 띠가 된다(자취를 덩이로 바꾼 까닭이 사라진다). */
  /* ★ **지름 2배 · 빈도 반**(2026-09, 요청: "미사일트레이서류의 연기 지름 2배로 키우고 대신
     빈도 반으로 낮추기") — 간격(puff) 9.33 → 18.66 이 곧 '빈도 반'이다(빈도 = 간격의 역수).
     반지름은 붓의 9.12(옛 4.56)이고 간격의 바닥(step9)도 2.0 이다 — **셋이 같은 2배**라야
     겹침 비가 그대로다. 탄두·배기 불꽃은 w 에 매여 있어 한 톨도 안 바뀐다(요청이 이름 붙인
     것만 — 그 규약). */
  missile: { dur: 0.28, w: 0.15, l: 24, puff: 18.66, smoke: "#e2ecfa", smokeEdge: "#8fc6ff", smokeFade: 0.95, warhead: "#a8b0bc", g: [[0, "rgba(220,232,248,1)"], [0.1, "rgba(196,218,244,0.94)"], [0.32, "rgba(176,200,232,0.6)"], [0.68, "rgba(154,172,196,0.32)"], [1, "rgba(134,150,174,0)"]], glow: "rgba(168,198,238,0.5)" },
  /* 스카우트의 대공탄 — 미사일과 한 몸이되 **탄두만 금색**이다(요청: "스카우트는 테란과
     달리 금색 탄두 나가기"). 프로토스의 무기는 은색 쇳덩이가 아니라는 결이다. */
  /* ★ **연기는 테란과 똑같다**(지적: "스카우트 연기가 테란하고 달라서 똑같게 해 달라고
     한 거야") — 갈리는 것은 **탄두 하나**다. 한때 속·테·배기까지 금빛으로 옮겨 봤는데,
     그러면 자취가 통째로 노래져 프로토스 무기가 아니라 '불타는 미사일'로 읽혔다.
     연기 값(smoke·smokeEdge·g·glow)은 missile에서 한 글자도 안 바꾸고 그대로 쓰고,
     warhead만 금(#c9a132)이다 — 그것이 요청("스카우트는 테란과 달리 금색 탄두")의 전부다.
     ※ 두 줄을 고칠 때는 **함께** 고쳐라: 연기가 갈리면 같은 무기가 두 결로 읽힌다. */
  missileG: { dur: 0.28, w: 0.15, l: 24, puff: 18.66, smoke: "#e2ecfa", smokeEdge: "#8fc6ff", smokeFade: 0.95, warhead: "#c9a132", g: [[0, "rgba(220,232,248,1)"], [0.1, "rgba(196,218,244,0.94)"], [0.32, "rgba(176,200,232,0.6)"], [0.68, "rgba(154,172,196,0.32)"], [1, "rgba(134,150,174,0)"]], glow: "rgba(168,198,238,0.5)" },
  /* ★ 디바우러의 산 — **뭉게뭉게한 연기 구름**이다(2026-09, 요청: "디바우러 트레이서도 연기
     느낌 나게 뭉게뭉게 하게 수정하고 약간 크기 키우기"). 매끈한 보라 띠는 산이 아니라 광선으로
     읽혔다. 굵기·길이를 한 뼘 키우고(1.3 → 1.6 · 2.25 → 2.6) 그 위에 보랏빛 덩이 열둘을 얹는다 —
     덩이는 줄기 처음부터 깔린다(화염과 달리 앞쪽만 연기인 것이 아니라 통째로 구름이다).
     ★ 크기 **40% 축소 뒤 20% 확대**(2026-09, 요청: "디바우러 트레이서 크기 40프로 축소" → "20프로 확대") —
       굵기 1.6 → 0.96 → 1.152 · 길이 2.6 → 1.56 → 1.872(옛 값의 0.72배). 덩이 지름은 굵기(w)에 매여 있어
       함께 움직인다(billowK 는 그대로). */
  acid: { dur: 0.75, w: 1.152, l: 1.872, g: [[0, "rgba(206,150,255,0.95)"], [0.2, "rgba(206,150,255,0.95)"], [0.6, "rgba(150,80,220,0.7)"], [1, "rgba(110,50,170,0)"]],
    billow: 12, billowFrom: 0.04, billowK: 0.92, billowA: 0.3, billowCol: "rgba(178,126,232,1)" },
  plasma: { dur: 0.24, w: 1.0, l: 2.75, g: [[0, "rgba(232,244,255,0.98)"], [0.18, "rgba(232,244,255,0.98)"], [0.55, "rgba(120,180,255,0.9)"], [1, "rgba(70,120,255,0)"]], glow: "rgba(120,180,255,0.55)" },
  ...
  /* 배틀크루저의 레이저 — 같은 빛인데 **훨씬 긴 선**이다(요청: "배틀크루저 트레이서 더 긴
     선으로 변경"). 레이스·스카웃과 갈래를 나눠 두는 까닭: 표 하나를 늘리면 셋이 함께
     길어져 작은 전투기의 탄이 함선의 것만큼 길어진다. 길이 2.4 → 6.2에 굵기를 0.3 → 0.26로
  ...
  /* ★ **일꾼 둘의 무기**(2026-09, 요청: "드론 침뱉기 프로브 지지기") ────────────────────
     · `spit` — 드론이 뱉는 산 한 방울. 디바우러의 `acid` 와 같은 결이되 **아주 작고 짧다**
       (일꾼의 침이지 폭격이 아니다). 날아가는 탄이라 PROJECTILE_FX 에 든다.
     · `weld` — 프로브의 입자 빔. 짧게 **지지직거리는 푸른 아크**라 꺾인 갈래(zig)를 쓰되,
       아콘과 달리 마디가 셋뿐이고 가늘다. 날아가지 않는 즉발이다. */
  /* ★ **야마토포**(2026-09, 요청: "배틀은 야마토를 말한 거였는데 그건 기타 액션에 넣어 줘") —
     지도에서 이것은 캐스트(표적 자리에 피어나는 빛덩이)라 갈래표에 없었다. 도록의 액션 칸이
     한 발을 그리려면 제 갈래가 있어야 하므로, **같은 색 결**(ReplayMotionPlayer 의 yamato 그러데이션)
     로 큰 광탄 하나를 적는다 — 흰 속에 푸른 테를 두른 덩이가 느릿하게 날아간다. */
  /* ★ **주황 혜성**이다(2026-09, 지적: "야마토 트레이서의 전체적으로 주황톤 불꽃이어야 함. 앞대가리 크기 키우고 꼬리는
     혜성 꼬리처럼 길게") — 머리는 흰 속에 노랑·주황 테를 두른 큰 덩이(w 2.6 → 3.4), 꼬리는 길게(l 2 → 9) 여위며
     (cone: 머리 1.0 → 꼬리 0.22 — 이 갈래는 머리가 0 쪽이라 cone[0] 이 머리다) 벌건 잔불로 꺼진다. */
  yamato: { dur: 0.85, w: 3.4, l: 9.0, cone: [1.0, 0.22], glow: "rgba(255,150,60,0.6)", glowW: 1.5, glowA: 0.55,
    g: [[0, "rgba(255,255,255,0.98)"], [0.1, "rgba(255,240,180,0.96)"], [0.28, "rgba(255,180,70,0.9)"], [0.55, "rgba(255,120,30,0.62)"], [0.8, "rgba(230,70,20,0.3)"], [1, "rgba(200,50,10,0)"]] },
  spit: { dur: 0.3, w: 0.42, l: 1.15, g: [[0, "rgba(206,255,150,0.95)"], [0.45, "rgba(150,220,80,0.8)"], [1, "rgba(104,168,40,0)"]] },
  weld: { muzzleLit: true, dur: 0.18, w: 0.4, l: 1.5, zig: 3, zigAmp: 0.18, zigW: 2.2, cap: "butt",
    glow: "rgba(120,190,255,0.85)", glowW: 1.8, glowA: 0.6,
    g: [[0, "rgba(255,255,255,1)"], [0.6, "rgba(206,236,255,0.95)"], [1, "rgba(150,200,255,0)"]] },
  ...
  /* ⚠ 마디 수를 **9 → 5** 로 줄인다(2026-09, 지적: "아콘 트레이서 줄기 너무 심하게 꺾임 ·
     꺾임각이 90도보다 크게 둔각으로") — 안쪽 각은 **마디 길이 대비 튀는 폭**이 정한다. 마디가
     짧을수록(9칸) 같은 폭이 더 날카롭게 꺾인다. 둔각을 지키는 자 자체는 붓에 있다(그 자리의 ★). */
  zap: { dur: 0.2, w: 1.75, l: 2.25, span: true, zig: 5, zigAmp: 0.30, zigW: 4.0, cap: "butt",
  ...
  venom: { dur: 0.26, w: 1.25, l: 2.5, g: [[0, "rgba(240,255,140,0.98)"], [0.2, "rgba(240,255,140,0.98)"], [0.55, "rgba(190,230,60,0.85)"], [1, "rgba(140,190,30,0)"]] },
...
export const UNIT_INK_CY9: Record<string, [number, number]> = { scv: [7.93, 8.72], gunner: [10.24, 10.94], ghost: [10.26, 10.95], fbat: [10.19, 10.90], inf: [10.26, 10.96], vulture: [9.30, 10.00], tank: [11.15, 11.62], goliath: [9.79, 10.49], wraith: [8.26, 9.07], dship: [8.41, 9.22], vessel: [9.59, 10.31], valk: [8.23, 9.03], bc: [9.16, 9.91], scvMin: [7.90, 8.74], scvGas: [7.83, 8.70], tanksiege: [11.04, 11.50], mine: [11.66, 12.28], probe: [10.08, 10.78], zealot: [9.50, 10.24], goon: [9.70, 10.42], htemp: [9.10, 9.83], dtemp: [10.10, 10.73], archon: [8.79, 9.57], darchon: [8.78, 9.56], shuttle: [8.39, 9.09], reaver: [10.65, 11.31], observer: [9.40, 10.13], scout: [8.01, 8.82], corsair: [8.69, 9.48], carrier: [9.37, 10.07], interceptor: [6.54, 7.45], scarab: [10.97, 11.63], arbiter: [8.32, 8.85], larva: [11.33, 11.96], egg: [11.28, 11.87], probeMin: [10.33, 11.00], probeGas: [10.34, 11.03], drone: [9.81, 10.53], ovie: [9.17, 9.94], zling: [10.38, 11.07], hydra: [8.93, 9.71], lurker: [9.74, 10.44], muta: [7.62, 8.53], scourge: [7.03, 7.91], queen: [7.84, 8.63], ultra: [7.95, 8.41], defiler: [11.19, 11.83], guardian: [7.64, 8.47], devourer: [9.00, 9.78], lurkeregg: [11.73, 12.26], mutacocoon: [9.51, 10.25], droneMin: [9.81, 10.53], droneGas: [9.78, 10.50], tankbody: [11.24, 11.77], tankgun: [9.79, 10.20], tanksiegebody: [11.18, 11.71], tanksiegegun: [9.63, 10.04], burrowhole: [11.83, 12.44] };
export const BLD_INK_MID9: Record<string, [number, number]> = { tomb: [4.56, 3.92], trapezoid: [2.78, 2.40], refinery: [4.04, 3.43], cube: [7.13, 6.53], ebay: [5.42, 4.57], tombFlat: [3.37, 2.77], academy: [4.14, 3.61], turret: [5.53, 4.93], factory: [4.31, 3.77], plane: [5.57, 4.82], armory: [4.14, 3.56], scifac: [3.31, 2.83], comsat: [3.06, 2.79], nsilo: [3.23, 2.84], mshop: [2.85, 2.50], ctower: [3.09, 2.63], covert: [2.40, 2.03], physlab: [2.60, 2.25], pyramidWide: [6.02, 5.10], diamond: [4.83, 4.25], assim: [2.88, 2.56], gate: [3.07, 2.62], forge: [3.98, 3.40], coil: [4.88, 4.02], sbattery: [3.05, 2.56], cyber: [3.19, 2.78], citadel: [4.07, 3.46], archives: [3.40, 2.82], dome: [4.10, 3.52], robobay: [3.05, 2.64], observatory: [3.07, 2.66], arch: [3.11, 2.70], fleetbeacon: [3.53, 3.09], tribunal: [3.05, 2.59], warpin: [4.48, 4.48], hatchery: [4.49, 3.81], lair: [5.62, 4.97], hive: [6.31, 5.67], creep: [3.96, 3.33], sunken: [4.64, 3.80], spore: [4.34, 3.76], extract: [4.65, 3.97], pool: [4.05, 3.24], evo: [4.46, 3.75], hydraden: [5.46, 4.88], spire: [6.53, 5.87], gspire: [10.23, 9.26], queensnest: [4.68, 4.00], nydus: [4.45, 3.72], cavern: [4.36, 3.57], dmound: [4.71, 3.89], cocoon: [2.37, 1.94], sunkenfire: [5.83, 5.05], mineral: [3.26, 2.75], mineralb: [4.05, 3.43], mineralc: [3.50, 2.93], geyser: [2.88, 3.18], nuke: [4.19, 3.89], storm: [3.15, 2.92], nukeblast: [5.08, 4.08], nukecloud: [6.69, 5.89], tankbody: [3.07, 2.69], tankgun: [1.79, 1.58], tanksiegebody: [3.60, 3.13], tanksiegegun: [1.95, 1.72], tanksiegelegs: [0.93, 0.78], addonlink: [3.95, 3.41], burrowhole: [2.67, 2.16], lurkerburrow: [2.67, 2.17], lurkerfire: [2.70, 2.21], creeppatch: [4.11, 3.33], creeppatch2: [4.11, 3.33], creeppatch3: [4.12, 3.34] };
...
export const FX_IMPACT: Record<string, {
  ...
  // 야마토 착탄 — 흰 속·주황 테가 넓게 퍼진다(줄기와 같은 결 — 2026-09 주황으로).
  yamato: { r: 1.6, g: [[0, "rgba(255,255,255,0.98)"], [0.3, "rgba(255,200,110,0.72)"], [0.7, "rgba(255,120,40,0.32)"], [1, "rgba(230,80,20,0)"]], ring: "rgba(255,190,120,0.5)" },
  spit: { r: 0.42, g: [[0, "rgba(222,255,176,0.92)"], [0.5, "rgba(150,220,80,0.5)"], [1, "rgba(104,168,40,0)"]] },
  weld: { r: 0.34, g: [[0, "rgba(255,255,255,0.95)"], [0.5, "rgba(170,215,255,0.5)"], [1, "rgba(120,180,255,0)"]] },
  ...
  venom: { r: 1.0, g: [[0, "rgba(250,255,190,0.95)"], [0.35, "rgba(200,240,70,0.6)"], [1, "rgba(140,190,30,0)"]] },
  acid: { r: 0.9, g: [[0, "rgba(238,210,255,0.95)"], [0.35, "rgba(180,110,240,0.6)"], [1, "rgba(110,50,170,0)"]] },
  plasma: { r: 0.95, g: [[0, "rgba(240,250,255,0.98)"], [0.34, "rgba(120,180,255,0.6)"], [1, "rgba(70,120,255,0)"]], ring: "rgba(170,215,255,0.55)" },
  ...
  tankshell: { r: 0.5, flat: 0.38, g: [[0, "rgba(255,255,255,1)"], [0.26, "rgba(255,247,200,1)"], [0.55, "rgba(255,201,92,0.92)"], [0.82, "rgba(255,142,42,0.52)"], [1, "rgba(255,110,20,0)"]] },
  ...
  const build = (t: number, still = false): Frame9 => {
    ...
    const rBD9 = buildsDrawOrder.map((i) => {
      ...
      if (raising) {
        ...
        /* ★ 연속 배수 성장은 걷었다(2026-09, 지적: "공사고치 단계별로 면적도 늘어나야지") — 성장은 **단계 모델**(cocoon1·2·cocoon,
           bake9 cocoonBuild9)이 맡는다. 드론에서 변태하는 자리는 다 자란 고치가 발자국을 넘겨 채우는 1.2(옛 끝값) 그대로,
           선 건물이 변태하는 자리는 1 이다. */
        const stage = BLD_FROM_BLD.has(unit) ? 1 : 1.2;
    ...
    /* ★ **태어난 순간에 닫힌 줄은 무너진 것이 아니다**(계측: 가스폭발[370.4s Assimilator ← 건물무너짐
       (sec=370.4 gone=370.4 done=Infinity)] · 익스트랙터도 같은 꼴) ─────────────────────────────
       가스 건물은 줄이 둘 생긴다 — 일꾼의 착공 앵커로 열린 생애와, 간헐천 개체가 건물로 바뀐 생애.
       앞엣것은 뒤엣것이 서는 순간 닫히는데(sec = gone, 완공 없음) 그 닫힘을 무너짐으로 읽어 착공
       자리에서 폭발이 났다(지적: "가스 지을 때 한 번 폭파되면서 짓기 시작" · "어시밀레이터도").
       선 적이 없는 줄(0.1초도 못 선 줄)은 무너질 것도 없다 — 폭발도 무너짐 연기도 안 낸다. */
    if (goneAt - sec < 0.1) return null;
...
replayTrack에서 문턱을 뒀다(초당 0.4타일 미만은 안 걷는 것으로 본다). */
    ...
    /* ★ 세로만이 아니라 **고치 한가운데까지**(지적: "드론이 공사고치로 변할 때랑 취소할 때 이동
       ...
       한가운데에 서는데 드론은 옆에서 사라졌다. 이제 드론의 자리에서 고치의 한가운데(발자국 가운데
    ...
    /* ★ **마엘스톰·이레디에이트는 걸린 몸마다 그린다**(2026-09, 요청: "원작과 비슷하게 — 웹은 영역이고 마엘스톰·이레디에이트는
       유닛별 · 마엘스톰은 광역이고 이레디에이트는 개별") — 원작의 마엘스톰은 시전 자리에 판이 서는 것이 아니라 걸린 몸마다
       주황 에너지 소용돌이가 감고, 이레디에이트는 그 몸에 초록 구름이 붙어 따라다닌다. 우리(cage)와 같은 자로 몸 한가운데에
       얹는다(세 몫의 높이 · 그려진 몸 폭). 시전 자리의 판(AREA_FX)은 마엘스톰이 짧은 섬광 하나, 이레디에이트는 없다. */
    {
      const stNow9 = e.statuses.find(([sa9, sb9, sk9]) => t >= sa9 && t < sb9 && (sk9 === "mael" || sk9 === "irr"));
      if (stNow9 && !markerView) {
        const cpx8 = drawUnit === "" ? unitGlyphPx(kindMain, kindMain, 0, ay3) : unitPxOf(drawUnit2, ay3, kindMain);
        fxOps.push({
          kind: "dom", style: "cast", sub: stNow9[2] === "mael" ? "maelU" : "irrU", fx, fy,
          lift: (uAir ? airLiftPxOf(ay3) : 0) + rise9 * cpx8 + cpx8 * 0.24,
          size: cpx8 * (modelInkOf(kindMain) / 16), age: t - stNow9[0], seed: e.tag % 97,
        });
      }
    }
    ...
    unitOps.push({
      ...
      tint: (() => {
        ...
        // 마엘스톰·이레디에이트는 몸에 제 효과를 씌우므로(위 maelU·irrU) 발밑 오라를 안 깐다 — 원작에도 원반이 없다.
        if (actSt[2] === "mael" || actSt[2] === "irr") return undefined;
    ...
    if (gunKind && !markerView && !liteView) {
      ...
      /* ★ 시즈 대기 포신은 **차체 뒤**다(지적: "시즈모드 처음 하면 포신은 시즈탱크 기준 뒤쪽 — 앞 포신이 들어가고 뒤
         포신이 나오는 설정"). 시즈 판(rotDeg = 앞 − 270 = 45)에서 포신 판은 rotDeg 그대로 보이므로 뒤(앞 + 180 = 135)
    ...
    /* ★ 스카우트의 **지상**은 트레이서가 아니라 포신의 스파크다(지적: "스카우트는 레이저
       ...
       번쩍이고, 길이가 짧아 포신 앞을 벗어나지 않는다.
       ★ **골리앗의 지상도 같다**(요청: "스파크 — 골리앗 지상 공격에도 적용해 줘") — 그래서
         셋 중 레이스만 표(ATTACK_FX의 laser)로 물러나고 나머지 둘은 spark다. */
    /* ★ 이 갈래는 **공용 문으로 뺐다**(2026-09, 요청: "공격 트레이서가 두 종류 이상(골리앗
       스카우트 배틀 등)인 경우 나눠서 두 셀로 표시") — 여태 여기 인라인으로 박혀 있어
       **도록이 그 값을 알 길이 없었다**(도록은 ATTACK_FX 한 줄만 보고 늘 지상 것만 그렸다).
       `attackFxOf9` 한 자리에 모아 지도와 도록이 같은 수를 본다. */
    const fxName9 = attackFxOf9(fxUnit, !!foe.air);
...
/* ★ 나는 시간에 **바닥을 깐다**(지적: "골리앗 뮤탈 상대로 트레이서 안 나감") ─────────
    ...
    if (fxName9 && NO_BEAM_FX.has(fxName9)) {
      ...
      if (beamLen > 1 && (MUZZLE_BURST_FX9.has(fxName9) || SHELL_ONLY_FX9.has(fxName9))) {
        /** 포구 불꽃·착탄 폭발을 내는가 — 탄만 나는 갈래(cannon)는 거짓이다(위 ★★). */
        const burst9 = MUZZLE_BURST_FX9.has(fxName9);
        ...
        if (uS9 < 1) {
          ...
          // ⚠ **작아야 총구에 있는 것으로 읽힌다**(지적: "폭발이 포구가 아니라 몸 중심에 있는
          //   거 같아") — 중심이 총구여도 지름이 몸만 하면 몸을 통째로 덮는다.
          if (burst9) put9(mzx9, mzy9, fxPx * 0.38, fxName9, 0.12 + uS9 * 0.88, beamDeg);
          ...
          /* ★★ 각은 **포물선의 접선이 아니라 총구 → 표적의 곧은 선**이다(2026-09, 지적:
             "시즈탱크 포탄 세로로 긴 게 아니라 가로로 긴 타원형이어야 하고") — 접선을 주면
             **쏘는 순간의 기울기가 거의 수직**이다(발사각 dy − 4·arc 에서 arc 가 사거리의
             0.12 라 세로 몫이 0.48·사거리다). 나는 몫이 0.16초뿐이라 눈에 드는 것은 그
             앞부분이고, 그래서 에너지포가 늘 세로로 선 타원으로 보였다.
             덩이의 긴 축은 **날아가는 길**을 가리켜야 하고, 그 길은 지도에서 총구와 표적을
             잇는 선이다 — 포물선은 그 위에 얹힌 들림일 뿐이다. */
          ...
          /* ★ 탱크 모드의 광전포는 **길 따라 길쭉**하다(2026-09, 요청: "타원형 광전포 … 이동방향으로 길쭉") —
             시즈의 원반(+90, 길을 가로지른다)과 달리 각을 안 돌린다. */
          put9(bx9, by9, fxPx * (burst9 ? 0.68 : 0.26), "tankshell", 0.3,
            (Math.atan2(-(tvx9 - mzx9), tvy9 - mzy9) * 180) / Math.PI + (burst9 ? 90 : 0));
        } else {
          /* ③ 착탄 스플래시 — 닿은 뒤 0.5초 동안 피었다 진다. 자는 쏘는 쪽 불꽃의 두 배가 넘는다
             (요청: "맞는 쪽도 똑같은 모양인데 더 크게(넓게) 나옴, 스플래시 데미지니까"). */
          const sp9 = (elS9 - shSec9) / 0.5;
          /* ⚠ **자를 한 단 줄였다**(2026-09, 지적: "탄착지 폭발 너무 큰 듯") — 1.5 는 쏘는 몸
             상자의 1.5배이고 거기에 갈래표의 반지름 1.55 가 또 곱해져(tankboom) 실제로는 **몸의
             2.3배**가 깔렸다. 스플래시는 '넓게'가 뜻이지 '표적을 삼키게'가 아니다 — 1.0(몸의
             1.55배)이면 여전히 포구 불꽃(0.38)의 2.6배라 그 뜻이 산다. */
          if (burst9 && sp9 <= 1) put9(tvx9, tvy9, fxPx * (fxName9 === "siege" ? 1.0 : 0.8), "tankboom", sp9);
        }
      } else if (beamLen > 1) {
        const [tfx9, tfy9] = posFrac(foe.bx, foe.by);
    ...
    if (lurkStrike) {
      ...
      for (const [lx9, ly9] of laneAt9) {
        fxOps.push({
          ...
          // 쌍이면 붓이 나란히 내보내 표적 가까이에서 모은다(FxOp.twin 의 ★).
          ...(laneAt9.length > 1 ? { twin: true } : {}),
        });
      }
      ...
      /* 시작은 표면 **살짝 안쪽**(요청: "아콘 몸 표면부터 그려지는 느낌" — 반지름보다 살짝 적게): 앵커(총구)는 중심 그대로고
         그려지는 시작만 반지름의 0.8 지점이다. 표적 쪽 끝(surf9)은 그대로. */
      const mzsx9 = mzx9 - Math.sin(rad9) * surf9 * 0.8;
      const mzsy9 = mzy9 + Math.cos(rad9) * surf9 * 0.8;
      /* ★ **화염은 양손 두 줄기다**(2026-09, 요청: "파이어뱃 불기둥 양쪽 건에서 각각 나와서
         총 두 개여야 함") — 파이어뱃은 팔 끝마다 화염방사기가 달렸는데 한 줄기로 그리면
      ...
      fxOps.push({
        ...
        mx: (st9Span(fxName9) ? mzsx9 : mzx9) + fdx9,
        my: (st9Span(fxName9) ? mzsy9 : mzy9) + fdy9,
        deg: beamDeg,
        ...(beamAt9.length > 1 ? { twin: true } : {}),
    ...
    }
    {
      ...
      const AREA_FX: Record<string, [string, number]> = {
        Plague: ["plague", 5], Ensnare: ["ensnare", 5],
        /* (걷어냄·2026-09) Irradiate — 시전 자리의 판이 아니라 **걸린 몸**에 초록 구름을 씌운다(유닛 op 곁의 irrU · 원작 그대로).
           표적을 되짚어 판을 몸에 얹던 옛 길도 함께 걷었다 — 상태(statuses)가 이미 그 몸을 안다. */
