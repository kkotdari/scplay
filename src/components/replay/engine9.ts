...
replayTrack에서 문턱을 뒀다(초당 0.4타일 미만은 안 걷는 것으로 본다). */
    ...
    if (gunKind && !markerView && !liteView) {
      ...
      /* ★ 시즈 대기 포신은 **차체 뒤**다(지적: "시즈모드 처음 하면 포신은 시즈탱크 기준 뒤쪽 — 앞 포신이 들어가고 뒤
         ...
         는 +90이다. 시즈로 가는 전환 창은 탱크 판(rotDeg = 앞)이라 +180, 언시즈 창은 +180에서 0(앞)으로 돌아온다. */
      const gunRest9 = kind0 === "tanksiege" ? 90
        : siegeXf9 && siegeXf9.to === 1 ? 180
          : siegeXf9 && siegeXf9.to === 0 ? 180 * (1 - Math.min(1, Math.max(0, siegeXf9.u))) : 0;
