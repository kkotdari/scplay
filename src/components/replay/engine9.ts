...
export const MODEL_NORM: Record<string, number> = {
  arbiter: 1.690,  // 상자 상한(원한 배수 2.224)
  archon: 0.481,  // 재측정(model-norm)
  bc: 0.597,
  burrowhole: 0.732,
  carrier: 0.646,
  corsair: 1.256,  // 재측정(model-norm) — 위팔 0.8배 뒤
  darchon: 0.495,
  defiler: 0.751,   // 0.687 → ×1.2(요청: 디파일러 그리기 1.2배)
  devourer: 0.784,
  drone: 0.966,  // 재측정(model-norm)
  droneGas: 0.934,
  droneMin: 0.959,
  dship: 0.664,  // 포드 축소·안쪽 이동 뒤 재측정(model-norm)
  dtemp: 0.856,  // 재측정(model-norm) — 망토를 1.2배로 늘리고 들어 올린 뒤(잉크 폭 5.48 → 6.26)
  egg: 1.224,   // 정수리를 둥글게 한 뒤 model-norm 재측정
  fbat: 1.215,
  ghost: 1.628,  // 상자 상한(원한 배수 1.723)
  goliath: 0.683,   // 재측정(model-norm) — 미사일 포드 걷고 포신 하나로(옛 0.671)
  goon: 0.623,  // 재측정(model-norm)
  guardian: 0.675,  // 다리 옆으로 곧게 뒤 재측정(model-norm)
  gunner: 1.314,  // 팔 길이 고정 뒤 model-norm 재측정
  htemp: 1.153,  // 재측정(model-norm) — 왕관(살 다섯·보석)까지 반영
  hydra: 0.652,  // 머리장식 확대·머리 위 시작·길이 +20%·폭 −20%·마름모 허리 0.75 뒤 재측정(model-norm의 **top 모드** 표)
  inf: 1.532,  // 상자 상한(원한 배수 1.615)
  interceptor: 1.413,  // 재측정(model-norm)
  larva: 1.267,  // 상자 상한(원한 배수 1.466)
  lurker: 0.547,
  lurkeregg: 0.820,
  mine: 0.745,  // 다리를 걷은 뒤 재측정은 1.840이지만 몸 크기를 지키려 옛 값 1.007 유지 → ×0.8(요청: "벌처랑 마인 0.8배")
  muta: 0.714,
  mutacocoon: 1.756,  // 상자 상한(원한 배수 1.891)
  observer: 1.853,
  ovie: 0.809,  // 앞턱을 호로 바꾼 뒤 재측정(model-norm)
  probe: 1.632,  // 다리 두께면을 양쪽으로 고친 뒤 model-norm 재측정
  probeGas: 1.435,
  probeMin: 1.486,
  queen: 0.588,
  reaver: 0.809,  // 재측정(model-norm)
  scarab: 1.452,  // 상자 상한(원한 배수 1.591)
  scourge: 1.203,  // 상자 상한(원한 배수 1.327)
  scout: 0.766,  // 재측정(model-norm)
  scv: 0.716,  // 하완 1.2배·상완 원통 걷은 뒤 재측정(model-norm) — 옛 0.767
  scvGas: 0.813,
  scvMin: 0.822,
  shuttle: 0.726,  // 재측정(model-norm) — 뒷다리를 더 오므린 뒤
  ...
  tanksiegebody: 0.690,
  ultra: 0.332,
  valk: 0.631,   // 앞동체 −10% 뒤 재측정(model-norm) 0.840 → ×0.8(요청: 발키리 그리기 0.8배)
  vessel: 0.872,  // 방패 접힘 축·뾰족 위끝 뒤 재측정(model-norm)
  vulture: 0.614,   // 0.833 × 0.8(요청: "벌처랑 마인 0.8배")
  wraith: 0.818,  // 재측정(model-norm)
  zealot: 0.977,  // 투구·관(마루 6.36)·묶음 재작도 뒤 재측정(model-norm의 top 모드) · 옛 0.955
  zling: 0.731,
...
export const BLD_NORM: Record<string, number> = {
  academy: 1.464,  // 치마형 받침으로 바꾼 뒤 bld-norm 재측정
  arch: 2.076,  // 판 0.9배 뒤 재측정 2.599 × 0.8(요청: 전체 그려지는 크기 0.8배)
  archives: 1.726,  // ×0.8(요청: 그려지는 크기 0.8배) · 옛 2.489
  armory: 1.217,
  assim: 1.533,  // 재작 뒤 재측정(bld-norm)
  cavern: 1.079,
  citadel: 1.519,  // 돛 두께 0.26·띠를 드럼/반구 따로 뒤 재측정(bld-norm 2.145) × 0.8(요청: 0.8배)
  cocoon: 2.039,
  coil: 1.333,
  comsat: 1.388,
  covert: 1.499,  // 사진 재작도 뒤 재측정(bld-norm)
  creep: 1.143,  // ×0.8(요청: 그리기 0.8배) · 발 셋 이식·요잉·촛대·촛대 가시 뒤 재측정 1.438 · 옛 1.232
  ctower: 1.538,
  ...
  cube: 1.005,   // 다리 안쪽·뒤쪽 앞으로 25%·기둥 20% 축소 뒤 재측정 0.998 × 1.2 · 옛 1.055
  cyber: 1.568,  // ×0.8(요청: 그려지는 크기 0.8배)
  diamond: 1.905,  // 상자 상한에 걸림
  dmound: 1.111,
  dome: 1.134,  // ×0.8(요청: 그려지는 크기 0.8배)
  ebay: 1.480,   // 다리 두 마디 20% 축소 뒤 재측정(잉크 폭이 좁아져 배수는 올라간다)
  evo: 1.167,  // ×0.8(요청: 그리기 0.8배) · 가운데 장기 경사·뒤 등걸 0.8 뒤 재측정 1.476 · 옛 1.232
  extract: 0.872,  // ×0.8(요청: 그리기 0.8배) · 창 2.5배(다섯) 뒤 재측정 1.066 · 옛 1.027
  factory: 1.573,  // 창·옆 홈·격납구·발판·굴뚝·쐐기 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  fleetbeacon: 1.654,  // ×0.8(요청: 그려지는 크기 0.8배)
  forge: 1.796,   // 돔 둘을 회전체로 바꾼 뒤 bld-norm 재측정
  gate: 1.966,  // 앞뒤 뿔탑 0.8배·발판에 붙인 뒤 재측정(bld-norm)
  geyser: 1.587,
  ...
  mineralb: 1.264,
  mineralc: 1.583,
  mshop: 1.570,  // −90° 요잉 뒤 재측정(bld-norm)
  nsilo: 1.343,
  nydus: 1.186,
  observatory: 1.879,  // 재측정 1.999 × 0.8(요청: 그려지는 크기 0.8배) — 바닥 관 제거·마름모 팔 뒤
  physlab: 1.509,  // 사진 재작도 뒤 재측정(bld-norm)
  plane: 1.255,   // 다리 낮춤·안테나 자 제외(boxSkip) 후 bld-norm 재측정
  pool: 1.189,  // ×0.8(요청: 그려지는 크기 0.8배) · 옛 1.449
  ...
  robobay: 1.539,  // 재측정 1.909 × 0.8(요청: 그려지는 크기 0.8배) — 가운데 결정을 작은 반구로 바꾼 뒤
  sbattery: 1.945,  // 빨대 다리 뒤 재측정(bld-norm)
  scifac: 1.465,  // 재작도 + 삼중탑 제거·왼판 축소 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  spire: 1.552,  // 상자 상한에 걸림
  spore: 1.333,  // ×0.8(요청: 그리기 0.8배) · 가운데 생체 포신 뒤 재측정 1.669 · 옛 1.333
  sunken: 0.949,  // ×0.8(요청: 그리기 0.8배) · 왼쪽 큰 발(길이 0.8배) 뒤 재측정 1.286 · 옛 1.046
  sunkenfire: 1.286,
