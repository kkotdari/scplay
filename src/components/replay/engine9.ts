...
export const BLD_NORM: Record<string, number> = {
  academy: 1.469,  // 치마형 받침으로 바꾼 뒤 bld-norm 재측정
  arch: 2.078,  // 판 0.9배 뒤 재측정 2.599 × 0.8(요청: 전체 그려지는 크기 0.8배)
  archives: 2.446,  // ×0.8(요청: 그려지는 크기 0.8배) · 옛 2.489
  armory: 1.427,
  assim: 1.829,  // 재작 뒤 재측정(bld-norm)
  cavern: 1.079,
  citadel: 1.720,  // 돛 두께 0.26·띠를 드럼/반구 따로 뒤 재측정(bld-norm 2.145) × 0.8(요청: 0.8배)
  cocoon: 2.063,
  coil: 1.333,
  comsat: 1.489,
  covert: 1.670,  // 사진 재작도 뒤 재측정(bld-norm)
  creep: 1.084,  // ×0.8(요청: 그리기 0.8배) · 발 셋 이식·요잉·촛대·촛대 가시 뒤 재측정 1.438 · 옛 1.232
  ctower: 1.823,
  ...
  cube: 1.084,   // 다리 안쪽·뒤쪽 앞으로 25%·기둥 20% 축소 뒤 재측정 0.998 × 1.2 · 옛 1.055
  cyber: 1.588,  // ×0.8(요청: 그려지는 크기 0.8배)
  diamond: 1.905,  // 상자 상한에 걸림
  dmound: 1.111,
  dome: 1.134,  // ×0.8(요청: 그려지는 크기 0.8배)
  ebay: 1.575,   // 다리 두 마디 20% 축소 뒤 재측정(잉크 폭이 좁아져 배수는 올라간다)
  evo: 1.229,  // ×0.8(요청: 그리기 0.8배) · 가운데 장기 경사·뒤 등걸 0.8 뒤 재측정 1.476 · 옛 1.232
  extract: 0.877,  // ×0.8(요청: 그리기 0.8배) · 창 2.5배(다섯) 뒤 재측정 1.066 · 옛 1.027
  factory: 1.674,  // 창·옆 홈·격납구·발판·굴뚝·쐐기 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  fleetbeacon: 1.872,  // ×0.8(요청: 그려지는 크기 0.8배)
  forge: 1.600,   // 돔 둘을 회전체로 바꾼 뒤 bld-norm 재측정
  gate: 2.073,  // 앞뒤 뿔탑 0.8배·발판에 붙인 뒤 재측정(bld-norm)
  geyser: 1.587,
  ...
  mineralb: 1.264,
  mineralc: 1.583,
  mshop: 1.482,  // −90° 요잉 뒤 재측정(bld-norm)
  nsilo: 1.484,
  nydus: 1.186,
  observatory: 1.311,  // 재측정 1.999 × 0.8(요청: 그려지는 크기 0.8배) — 바닥 관 제거·마름모 팔 뒤
  physlab: 1.671,  // 사진 재작도 뒤 재측정(bld-norm)
  plane: 1.124,   // 다리 낮춤·안테나 자 제외(boxSkip) 후 bld-norm 재측정
  pool: 1.161,  // ×0.8(요청: 그려지는 크기 0.8배) · 옛 1.449
  pyramidWide: 1.040,  // 재측정(bld-norm) — 기둥 5.5·원기둥 받침·표창 길이 원복 뒤
  queensnest: 1.145,   // 재측정(bld-norm) — 돔 5.6으로 키운 뒤(옛 1.148)
  refinery: 1.619,  // 아래판 뒤를 줄이고 뒤 드럼을 앞으로 당긴 뒤 bld-norm 재측정(1.394 →)
  robobay: 1.539,  // 재측정 1.909 × 0.8(요청: 그려지는 크기 0.8배) — 가운데 결정을 작은 반구로 바꾼 뒤
  sbattery: 1.945,  // 빨대 다리 뒤 재측정(bld-norm)
  scifac: 1.533,  // 재작도 + 삼중탑 제거·왼판 축소 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  spire: 1.539,  // 상자 상한에 걸림
  spore: 1.333,  // ×0.8(요청: 그리기 0.8배) · 가운데 생체 포신 뒤 재측정 1.669 · 옛 1.333
  sunken: 1.047,  // ×0.8(요청: 그리기 0.8배) · 왼쪽 큰 발(길이 0.8배) 뒤 재측정 1.286 · 옛 1.046
  sunkenfire: 1.406,
  tomb: 1.614,   // 옆 구리띠에 두께를 준 뒤 재측정 · 옛 1.534
  tombFlat: 1.779,   // 높이 1.1배·계단 2/3·그리기 크기 원복 뒤 재측정
  trapezoid: 2.560,
  tribunal: 1.681,  // ×0.8(요청: 그려지는 크기 0.8배)
  turret: 1.879,  // 포드를 팔자로 굴린 뒤 재측정(bld-norm) — 잉크 폭 9.05 → 8.98
  warpin: 2.196,
};
