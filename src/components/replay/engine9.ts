...
export const MODEL_NORM: Record<string, number> = {
  arbiter: 1.740,  // 상자 상한(원한 배수 2.224) · 날개 뒤끝 안으로 휘고 안쪽 평면 뒤 ×1.0435(2.229 → 2.326) → ×(2.305/2.331) 모델 z 0.8 손질 재측정
  archon: 0.480,  // 재측정(model-norm)
  bc: 0.620,   // 날개 스팬 20% 축소·동체 20% 축소 뒤 재측정 비(0.619 → 0.662)로 되올림 · 로켓 십자 깃(0.665) · 추진체 모음(0.656) · 미사일 뒤로(0.635) · 화살촉(0.632) · 깃 폭 반(0.653) · 관 0.11 뒤 ×1.0046(0.653 → 0.656) · 옛 0.596 → ×(0.646/0.656) 모델 z 0.8 손질 재측정
  burrowhole: 0.732,
  carrier: 0.630,   // → ×(0.649/0.663) 모델 z 0.8 손질 재측정
  corsair: 1.239,  // 재측정(model-norm) — 위팔 0.8배 뒤 → ×(1.277/1.289) 모델 z 0.8 손질 재측정
  darchon: 0.496,
  ...
  devourer: 0.774,   // → ×(0.880/0.887) 모델 z 0.8 손질 재측정
  drone: 0.963,  // 재측정(model-norm)
  droneGas: 0.933,
  droneMin: 0.958,
  dship: 0.652,  // 포드 축소·안쪽 이동 뒤 재측정(model-norm) → ×(0.674/0.683) 모델 z 0.8 손질 재측정
  dtemp: 0.813,  // 재측정(model-norm) — 망토를 1.2배로 늘리고 들어 올린 뒤(잉크 폭 5.48 → 6.26) → ×(0.827/0.862) 모델 z 0.8 손질 재측정
  egg: 1.210,   // 정수리를 둥글게 한 뒤 model-norm 재측정
  fbat: 1.197,   // → ×(1.222/1.230) 모델 z 0.8 손질 재측정
  ghost: 1.571,  // 상자 상한(원한 배수 1.723) → ×(1.719/1.752) 모델 z 0.8 손질 재측정
  goliath: 0.678,   // 재측정(model-norm) — 미사일 포드 걷고 포신 하나로(옛 0.671)
  goon: 0.591,  // 재측정(model-norm) → ×(0.619/0.648) 모델 z 0.8 손질 재측정
  guardian: 0.675,  // 다리 옆으로 곧게 뒤 재측정(model-norm) · … · 다리 끝 뾰족(0.672, 0.683 → 0.701) · 날개껍질 내리고 척추 앞 반 뒤 ×1.0043(0.701 → 0.704)
  gunner: 1.275,  // 팔 길이 고정 뒤 model-norm 재측정 → ×(1.298/1.326) 모델 z 0.8 손질 재측정
  htemp: 1.108,  // 재측정(model-norm) — 왕관(살 다섯·보석)까지 반영 → ×(1.117/1.149) 모델 z 0.8 손질 재측정
  hydra: 0.646,  // 머리장식 확대·머리 위 시작·길이 +20%·폭 −20%·마름모 허리 0.75 뒤 재측정(model-norm의 **top 모드** 표)
  inf: 1.483,  // 상자 상한(원한 배수 1.615) → ×(1.615/1.650) 모델 z 0.8 손질 재측정
  interceptor: 1.410,  // 재측정(model-norm)
  larva: 1.268,  // 상자 상한(원한 배수 1.466)
  lurker: 0.545,
  lurkeregg: 0.817,
  mine: 0.744,  // 다리를 걷은 뒤 재측정은 1.840이지만 몸 크기를 지키려 옛 값 1.007 유지 → ×0.8(요청: "벌처랑 마인 0.8배")
  muta: 0.710,
  mutacocoon: 1.701,  // 상자 상한(원한 배수 1.891) → ×(1.802/1.848) 모델 z 0.8 손질 재측정
  observer: 1.824,   // 1.835 → ×(1.859/1.861) 구 몸·짧은 드럼·날개 띠 → ×(1.872/1.859) 도넛 제거·세로 1.3배 → ×(1.885/1.872) 세로 1.0 복귀(실측 1.872→1.885) → ×(1.814/1.885) 적도 링(실측 1.885→1.814) → ×(1.849/1.814) 링 90도 피칭(실측 1.814→1.849)
  ovie: 0.750,  // 앞턱을 호로 바꾼 뒤 재측정(model-norm) · 등 혹 넷 걷고 꽁무니 주머니 뒤 0.792(0.813 → 0.804) · 주머니 타원·배 임자색 뒤 ×1.0062(0.804 → 0.809) → ×(0.761/0.809) 모델 z 0.8 손질 재측정
  probe: 1.625,  // 다리 두께면을 양쪽으로 고친 뒤 model-norm 재측정
  probeGas: 1.369,   // → ×(1.274/1.327) 모델 z 0.8 손질 재측정
  probeMin: 1.421,   // → ×(1.352/1.403) 모델 z 0.8 손질 재측정
  queen: 0.585,
  reaver: 0.786,  // 재측정(model-norm) → ×(0.806/0.823) 모델 z 0.8 손질 재측정
  scarab: 1.451,  // 상자 상한(원한 배수 1.591)
  scourge: 1.239,  // 상자 상한(원한 배수 1.327) · 재작도 1.174 · 팔 바깥 1.095 · 피칭 1.133(1.159 → 1.199) · 손톱 안쪽 뒤 ×1.1226(1.199 → 1.346) → ×(1.311/1.346) 모델 z 0.8 손질 재측정
  scout: 0.764,  // 재측정(model-norm)
  scv: 0.712,  // 하완 1.2배·상완 원통 걷은 뒤 재측정(model-norm) — 옛 0.767
  scvGas: 0.793,   // → ×(0.830/0.846) 모델 z 0.8 손질 재측정
  scvMin: 0.801,   // → ×(0.835/0.852) 모델 z 0.8 손질 재측정
  shuttle: 0.725,  // 재측정(model-norm) — 뒷다리를 더 오므린 뒤
  ...
  tank: 0.702,
  tankbody: 0.672,   // → ×(0.695/0.711) 모델 z 0.8 손질 재측정
  tanksiege: 0.695,
  tanksiegebody: 0.683,   // → ×(0.625/0.629) 모델 z 0.8 손질 재측정
  ultra: 0.365,   // 팔뚝을 걷고 낫을 몸에 붙인 뒤 ×1.1134(0.344 → 0.383) → ×(0.379/0.383) 모델 z 0.8 손질 재측정
  valk: 0.603,   // 앞동체 −10% 뒤 재측정(model-norm) 0.840 → ×0.8(요청: 발키리 그리기 0.8배) · 추진체 연장분 ×0.991 → ×(0.775/0.802) 모델 z 0.8 손질 재측정
  vessel: 0.869,  // 방패 접힘 축·뾰족 위끝 뒤 재측정(model-norm)
  vulture: 0.612,   // 0.833 × 0.8(요청: "벌처랑 마인 0.8배")
  wraith: 0.817,  // 재측정(model-norm)
  zealot: 0.891,  // 투구·관(마루 6.36)·묶음 재작도 뒤 재측정(model-norm의 top 모드) · 옛 0.955 → ×(0.901/0.979) 모델 z 0.8 손질 재측정
  zling: 0.727,
...
export const BLD_NORM: Record<string, number> = {
  ...
  factory: 1.522,  // 창·옆 홈·격납구·발판·굴뚝·쐐기 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  ...
  mineralb: 1.264,
  mineralc: 1.583,
  mshop: 1.522,  // −90° 요잉 뒤 재측정(bld-norm)
  nsilo: 1.354,
  nydus: 1.184,
  ...
  scifac: 1.426,  // 재작도 + 삼중탑 제거·왼판 축소 · 다리 기둥 20% 축소 뒤 재측정(bld-norm)
  ...
  turret: 1.899,  // 포드를 팔자로 굴린 뒤 재측정(bld-norm) — 잉크 폭 9.05 → 8.98
  warpin: 2.196,
};
...
export const FX_BEAM: Record<string, {
  ...
  missile: { dur: 0.28, w: 0.15, l: 24, puff: 8.4, smoke: "#e2ecfa", smokeEdge: "#8fc6ff", smokeFade: 0.95, warhead: "#a8b0bc", g: [[0, "rgba(220,232,248,1)"], [0.1, "rgba(196,218,244,0.94)"], [0.32, "rgba(176,200,232,0.6)"], [0.68, "rgba(154,172,196,0.32)"], [1, "rgba(134,150,174,0)"]], glow: "rgba(168,198,238,0.5)" },
  ...
  missileG: { dur: 0.28, w: 0.15, l: 24, puff: 8.4, smoke: "#e2ecfa", smokeEdge: "#8fc6ff", smokeFade: 0.95, warhead: "#c9a132", g: [[0, "rgba(220,232,248,1)"], [0.1, "rgba(196,218,244,0.94)"], [0.32, "rgba(176,200,232,0.6)"], [0.68, "rgba(154,172,196,0.32)"], [1, "rgba(134,150,174,0)"]], glow: "rgba(168,198,238,0.5)" },
...
export const UNIT_INK_CY9: Record<string, [number, number]> = { scv: [7.41, 8.28], gunner: [10.32, 11.03], ghost: [10.41, 11.09], fbat: [10.33, 11.03], inf: [10.38, 11.08], vulture: [9.24, 9.95], tank: [11.05, 11.67], goliath: [9.79, 10.49], wraith: [8.19, 9.00], dship: [8.30, 9.10], vessel: [9.95, 10.65], valk: [7.83, 8.65], bc: [8.83, 9.59], scvMin: [7.51, 8.38], scvGas: [7.56, 8.42], tanksiege: [10.93, 11.58], mine: [11.66, 12.28], probe: [10.52, 11.19], zealot: [9.47, 10.20], goon: [9.42, 10.15], htemp: [9.22, 9.96], dtemp: [9.97, 10.64], archon: [8.79, 9.57], darchon: [8.78, 9.56], shuttle: [7.95, 8.73], reaver: [10.65, 11.31], observer: [9.67, 10.39], scout: [7.85, 8.67], corsair: [8.67, 9.47], carrier: [9.49, 10.22], interceptor: [6.08, 7.01], scarab: [10.97, 11.63], arbiter: [10.90, 11.56], larva: [11.33, 11.96], egg: [10.77, 11.43], probeMin: [10.22, 10.91], probeGas: [9.97, 10.67], drone: [9.77, 10.49], ovie: [8.75, 9.55], zling: [10.24, 10.93], hydra: [8.86, 9.62], lurker: [9.74, 10.44], muta: [7.58, 8.49], scourge: [6.55, 7.45], queen: [7.47, 8.29], ultra: [9.17, 9.91], defiler: [11.19, 11.83], guardian: [6.94, 7.80], devourer: [8.88, 9.69], lurkeregg: [11.73, 12.26], mutacocoon: [9.61, 10.34], droneMin: [9.80, 10.52], droneGas: [9.77, 10.49], tankbody: [11.09, 11.73], tankgun: [9.82, 10.52], tanksiegebody: [11.07, 11.72], tanksiegegun: [9.59, 10.31], burrowhole: [11.83, 12.44] };
export const BLD_INK_MID9: Record<string, [number, number]> = { tomb: [4.56, 3.94], trapezoid: [2.59, 2.21], refinery: [4.11, 3.62], cube: [7.46, 6.45], ebay: [5.42, 4.57], tombFlat: [3.37, 2.77], academy: [4.14, 3.61], turret: [5.67, 5.06], factory: [4.36, 3.77], plane: [5.79, 4.93], armory: [4.14, 3.56], scifac: [3.54, 3.00], comsat: [3.20, 2.79], nsilo: [3.23, 2.84], mshop: [2.86, 2.50], ctower: [2.98, 2.63], covert: [2.40, 2.03], physlab: [2.60, 2.25], pyramidWide: [6.05, 5.11], diamond: [4.68, 4.22], assim: [3.82, 3.40], gate: [3.10, 2.65], forge: [4.17, 3.60], coil: [4.88, 4.02], sbattery: [2.60, 2.16], cyber: [3.19, 2.79], citadel: [4.07, 3.46], archives: [3.40, 2.82], dome: [4.10, 3.52], robobay: [3.05, 2.64], observatory: [3.32, 2.91], arch: [3.11, 2.70], fleetbeacon: [3.49, 3.00], tribunal: [3.05, 2.58], warpin: [4.48, 4.48], hatchery: [4.70, 4.00], lair: [5.41, 4.63], hive: [5.95, 5.13], creep: [4.37, 3.64], sunken: [5.16, 4.31], spore: [4.31, 3.73], extract: [4.74, 4.07], pool: [4.46, 3.63], evo: [4.47, 3.75], hydraden: [5.87, 5.30], spire: [6.41, 5.98], gspire: [10.22, 9.26], queensnest: [4.73, 4.02], nydus: [4.32, 3.58], cavern: [4.98, 4.20], dmound: [4.58, 3.83], cocoon: [2.52, 2.13], sunkenfire: [6.56, 5.77], mineral: [3.26, 2.75], mineralb: [4.05, 3.43], mineralc: [3.50, 2.93], geyser: [3.88, 3.47], nuke: [4.20, 3.90], storm: [3.85, 3.57], nukeblast: [5.08, 4.08], nukecloud: [6.69, 5.89], tankbody: [3.22, 2.74], tankgun: [1.75, 1.48], tanksiegebody: [3.71, 3.13], tanksiegegun: [1.88, 1.59], tanksiegelegs: [0.93, 0.76], addonlink: [3.95, 3.41], burrowhole: [2.67, 2.16], lurkerburrow: [2.67, 2.17], lurkerfire: [2.70, 2.21], creeppatch: [4.11, 3.33], creeppatch2: [4.11, 3.33], creeppatch3: [4.12, 3.34] };
