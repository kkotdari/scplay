...
export function groundSquashNow(): number {
  /* 0.66 → 0.55(수리: 넥서스 앞 바닥·기둥이 뷰박스 밖으로 잘렸다) — 앞쪽 깊이가
     ...
     ★ 0.55 → 0.50(요청: "2D 시점을 원작에 맞추기") — 평면 보기만 **지면과 높이가 서로
       ...
       고칠 값은 **한 쌍**이어야 한다(sin θ, cos θ). 처음엔 지도 타일의 2:1을 그대로 믿고
       30도(0.5 / 0.866)로 뒀는데, 눈으로 보니 건물이 너무 솟고 시점이 낮았다(지적:
       "원작은 저렇게 높지도 않고 시점도 더 높은 데서 내려다보는 것 같은데") — **지형
       타일의 각과 스프라이트를 그린 각은 다르다.** 원작 스프라이트는 타일보다 훨씬 위에서
       내려다본 그림이다. 그래서 45도 한 쌍으로 둔다: 납작비 sin45 = cos45 = 0.707.
       옛 값과 견주면 바닥은 더 둥글어지고(0.55 → 0.707) 높이 대 깊이의 비는
       1.20 → 1.00으로 낮아져, 같은 건물이 더 납작하게 앉는다. */
...
function zScaleNow(): number {
  ...
  return pitchView ? 0.9 : topView ? 0.707 : 0.89;   // 0.8 → 0.9(재요청) · 평면 0.66 → cos45 = 0.707(요청: 시점을 더 높이)
}
