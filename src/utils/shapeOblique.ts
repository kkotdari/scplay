...
/* ★★ **0 이면 끈다 — 지금은 꺼 두었다**(2026-09, 요청: "원근끄자") ────────────────
   ⚠ 이 자는 **원근이 아니라 반쪽 원근**이었다: 가로(X)만 깊이로 나누고 세로(Y)는 1차식
   그대로라, 분모가 한쪽에만 있어 **모형의 직선이 화면에서 쌍곡선**이 됐다(진짜 사영변환은
   직선을 직선으로 보낸다 — 사진의 건물 모서리는 모여들기만 하고 휘지 않는다).
   실측(요잉 40 · 같은 모형 직선 위의 두 부품이 화면에서 갈린 각):
     · 배럭 좌우 벤트 앞선  26.07° vs 31.01°(**4.95°** · 현은 28.42°)
     · 스타포트 목 vs 꼬리  −31.79° vs −42.40°(**10.61°** · 처짐 0.401 모형칸)
   끄면 셋이 다 28.34°·−37.45° 한 각이다. 지적: "유독 대각으로 보여지는 직선상 부품들이
   직선이 아니게 보여" — 화면 가로로 누운 선은 ry 가 일정해 f 가 상수라 곧게 남고,
   **대각으로 보이는 선 = 깊이가 변하는 선**이라 전부 휘었다.
   · 세로도 같은 f 로 나누면(진짜 사영변환) 휨은 0 이 되지만 **나란히 선 건물의 이음매가
     세로로 최대 0.47 타일 벌어진다**(건물마다 제 원점을 눈 축으로 삼으므로 같은 세계 점을
     둘이 반대 방향으로 왜곡한다 · 지금 세로 차는 정확히 0). 그래서 그 길은 안 갔다.
   · 켜려면 이 값 하나다(옛 값 48 · 그전 30 — "모델 원근이 과함"으로 눅인 값). gl9 의
     셰이더·`footOf`·`modelXY9` 가 이 값을 읽으므로 두 붓이 한 자다. */
export const MODEL_PERSP = 0;
...
export const EMIT_FILL9 = new Set<string>(["#e4f6ff", "#5fe6ff", "#c4f4ff", "#7fd6ff", "#5cf08a", "#c9fff7"]);   // 뒤의 둘은 서플라이 격납구 속의 초록 불·로보틱스 눈알 심(늘 켜 둔 빛이라 winLit 을 안 지난다)
