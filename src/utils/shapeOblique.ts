...
export function groundSquashNow(): number {
  /* 0.66 → 0.55(수리: 넥서스 앞 바닥·기둥이 뷰박스 밖으로 잘렸다) — 앞쪽 깊이가
     ...
     ★ 0.55 → 0.50(요청: "2D 시점을 원작에 맞추기") — 평면 보기만 **지면과 높이가 서로
       ...
       한 단 위인 **45도** 한 쌍으로 둔다: 0.707 / 0.707. 높이 대 깊이의 비가
       1.73 → 1.00으로 떨어져 같은 모델이 확연히 눌려 앉는다.
...
function zScaleNow(): number {
  ...
  return pitchView ? 0.9 : topView ? 0.707 : 0.89;   // 0.8 → 0.9(재요청) · 평면 0.66 → cos45 = 0.707(실측 30도 위로 한 단)
}
