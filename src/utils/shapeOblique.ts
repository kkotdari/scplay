...
export const TOP_Z_PRESS9 = 0.8;   // 0.82 → 0.75 → 0.8(요청)
/* ── 굽는 요잉 칸 ────────────────────────────────────────────────────────────────
...
export function groundSquashNow(): number {
  /* 0.66 → 0.55(수리: 넥서스 앞 바닥·기둥이 뷰박스 밖으로 잘렸다) — 앞쪽 깊이가
     ...
     ★ 0.55 → 0.50(요청: "2D 시점을 원작에 맞추기") — 평면 보기만 **지면과 높이가 서로
       ...
       한 단 위로 올려 둔다 — 지금은 **30도** 한 쌍이다: 0.5 / 0.866 — 곧 발판 타원에서 잰 그 값으로 돌아왔다
       (45 → 40 → 35 → 30). 실측 그대로 두되 높이는 TOP_Z_PRESS9로 따로 눌러, '원작과 같은
  ...
  return pitchView ? pitchSquash : topView ? 0.5 : GROUND_SQUASH;
}
function zScaleNow(): number {
  ...
  return pitchView ? 0.9 : topView ? 0.866 * TOP_Z_PRESS9 : 0.89;   // 0.8 → 0.9(재요청)
}
