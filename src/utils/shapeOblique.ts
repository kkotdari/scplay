...
export function groundSquashNow(): number {
  /* 0.66 → 0.55(수리: 넥서스 앞 바닥·기둥이 뷰박스 밖으로 잘렸다) — 앞쪽 깊이가
     ...
     ★ 0.55 → 0.50(요청: "2D 시점을 원작에 맞추기") — 평면 보기만 **지면과 높이가 서로
       ...
       한 단 위로 올려 둔다 — 지금은 **35도** 한 쌍이다: 0.574 / 0.819(요청으로 45 → 40 → 35 — 수평 40은 그대로). 높이 대 깊이의 비가
  ...
  return pitchView ? pitchSquash : topView ? 0.574 : GROUND_SQUASH;
}
function zScaleNow(): number {
  ...
  return pitchView ? 0.9 : topView ? 0.819 * TOP_Z_PRESS9 : 0.89;   // 0.8 → 0.9(재요청)
}
